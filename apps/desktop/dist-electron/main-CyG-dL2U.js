...
function tokenizeSearchTerms(text) {
  const tokens2 = text.toLowerCase().replace(/[^\w\s-]/g, " ").split(/\s+/).filter((token) => token.length > 1 && !STOP_WORDS.has(token));
...
function tokenizeQuery(query) {
  ...
  const tokens2 = tokenizeSearchTerms(trimmed);
  if (tokens2.length === 0 && phrase.length > 0) {
    return [phrase];
  }
  return tokens2;
}
...
function pairRecordsBySharedTokens(records, relationships, seen, options) {
  ...
  for (const record of records) {
    const tokens2 = titleTokens(record);
    tokenSets.set(record.id, tokens2);
...
function searchRelationships(index, query, limit = 50) {
  const tokens2 = tokenizeQuery(query);
  const q = query.toLowerCase();
  const scored = [];
  for (const rel of index.relationships) {
    ...
    if (hay.includes(q))
      score += 20;
    score += tokens2.filter((t) => hay.includes(t)).length * 8;
    if (score > 0)
...
function tokens(text) {
  return new Set(sanitizeGist(text).toLowerCase().split(/[^a-z0-9]+/).filter((word) => word.length > 3));
}
...
function pushUnique(parts, line) {
  const trimmed = sanitizeGist(line);
  if (!trimmed)
    return;
  if (parts.some((existing) => substantiallyOverlaps(existing, trimmed)))
    return;
  parts.push(trimmed);
}
...
function lastSession(context) {
  ...
  if (!detail)
    return `Last session we were on ${title}.`;
...
function whatChanged(context, confidence) {
  ...
  switch (context.intent) {
    case "decision": {
      const session = context.executiveSessions[0];
      if (session)
        return gist(session.excerpt, 220);
      break;
    }
    case "summarize": {
      const throughLine = gist(context.items.slice(0, 3).map((item) => item.excerpt).join(" "), 240);
...
function whatNeedsAttention(context) {
  ...
  for (const hit of blockerHits.slice(0, 3)) {
    ...
    if (line && !tasks.some((task) => substantiallyOverlaps(task, line)))
      tasks.push(line);
  }
...
function recommendNext(context, confidence) {
  ...
  if (context.intent === "blockers") {
    return lead ? "Decide if this stays on your tracker, assign an owner, and set the next checkpoint." : "Clarify which campaign this affects so we can pick the next move.";
  }
...
function formatAttention(tasks) {
  if (tasks.length === 0)
    return null;
  if (tasks.length === 1)
    return `One thing needs your call — ${tasks[0]}.`;
...
function buildSteeringSupportingSummary(context, confidence) {
  ...
  const confidenceNote = confidence.level === "high" ? "I'm confident in this read" : confidence.level === "medium" ? "This read is moderate confidence" : confidence.level === "low" ? "Treat this as a lower-confidence read" : "Treat this as an early read";
  const threadCount = context.items.length;
  const sessionNote = context.executiveSessions.length > 0 ? ` — ${context.executiveSessions.length} prior session(s) align.` : "";
...
async function core() {
  const mod = await import("./index-Bvl92_jI.js");
