...
function gist(text, max = 220) {
  var _a, _b;
  const normalized = text.trim().replace(/^#+\s*/gm, "").replace(/\*\*([^*]+)\*\*/g, "$1").replace(/\s+/g, " ");
  if (!normalized)
    return "";
...
function whatChanged(context, confidence) {
  ...
  switch (context.intent) {
    ...
    case "summarize": {
      ...
      return `Across ${context.items.length} retrieved record(s) (${krcList}), the through-line is — ${gist(context.items.slice(0, 3).map((item) => item.excerpt).join(" "), 280)}`;
    }
    case "show_evidence": {
  ...
  const corroboration = context.topKrcIds.length > 1 ? ` with ${context.topKrcIds.length} corroborating sources` : "";
  return `"${lead.title}" is the lead finding${corroboration} — ${gist(lead.excerpt)}`;
}
function whyItMatters(context, confidence) {
...
function recommendNext(context, confidence) {
  ...
  const lead = leadItem(context);
  const session = context.executiveSessions[0];
  const explorer = (_a = context.items.find((item) => item.explorerPath)) == null ? void 0 : _a.explorerPath;
  if (context.intent === "blockers") {
    return lead ? `Open "${lead.title}" and decide whether to add this blocker to your active campaign tracker.` : "Ask me which campaign this blocker affects, then we can trace ownership.";
  }
  ...
  if (lead == null ? void 0 : lead.explorerPath) {
...
function buildSteeringDirectAnswer(context, confidence) {
  ...
  return [
    ...
    "Pick a lens below — I will open the right view for this investigation."
  ].join("\n\n");
}
function sanitizeGist(text) {
  return text.replace(/^#+\s*/gm, "").replace(/\*\*([^*]+)\*\*/g, "$1").replace(/\s+/g, " ").trim();
}
function buildSteeringSupportingSummary(context, confidence) {
  ...
  const confidenceNote = confidence.level === "high" ? "High confidence" : confidence.level === "medium" ? "Medium confidence" : confidence.level === "low" ? "Lower confidence" : "Limited confidence";
...
async function core() {
  const mod = await import("./index-CwkvNb1l.js");
