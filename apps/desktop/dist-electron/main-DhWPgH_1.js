...
import { safeStorage as tn, app as $e, protocol as Ts, BrowserWindow as nn, ipcMain as N, shell as $t, clipboard as ui, dialog as _n } from "electron";
...
function Di() {
  ...
  return Tt = c, c.prototype.makeDir = function(a) {
    const l = this;
    function d(u) {
    ...
    p.fs.exists(a, function(h) {
      ...
      p.fs.stat(a, function(I, b) {
        ...
        p.fs.exists(C, function(A) {
          A || p.makeDir(C), p.fs.open(a, "w", 438, function(D, g) {
            D ? p.fs.chmod(a, 438, function() {
    ...
    d.fs.readdir(a, function(f, p) {
      ...
      p.forEach(function(I) {
        I = e.join(a, I), d.fs.stat(I, function(b, C) {
          if (b) return l(b);
          C && (u.push(e.normalize(I) + (C.isDirectory() ? d.sep : "")), C.isDirectory() ? d.findFilesAsync(I, function(A, D) {
            if (A) return l(A);
            u = u.concat(D), --h || l(null, u);
          }) : --h || l(null, u));
        });
      });
    });
...
var Mn;
function Je() {
  return Mn || (Mn = 1, de.exports = Di(), de.exports.Constants = Bs(), de.exports.Errors = ln(), de.exports.FileAttr = xi(), de.exports.decoder = Ti()), de.exports;
}
var ot = {}, Lt, jn;
function _i() {
  if (jn) return Lt;
  ...
  return Lt = function() {
    ...
    const C = {
      extraLen: 0
    }, A = (g) => Math.max(0, g) >>> 0, D = (g) => Math.max(0, g) & 255;
...
function Fi() {
  ...
  function f(p, h, I, b = !1) {
    ...
    const D = Buffer.alloc(p.length + 12);
    return C(A, D), C(p, D, 12);
  }
  return Ut = { decrypt: d, encrypt: f, _salter: u }, Ut;
}
var Zn;
function Oi() {
    ...
    function A(y, w, E) {
      ...
      switch (o.method) {
        ...
        case t.Constants.DEFLATED:
          ...
          if (y)
            U.inflateAsync(function(x) {
              x.copy(x, 0), w && (C(x) ? w(x) : w(x, t.Errors.BAD_CRC()));
            });
          else {
  ...
  return jt = function(s, r) {
    ...
    function h() {
      ...
      for (const D of A)
        if (!(D in o)) {
          const g = new t(u);
          g.entryName = D, g.attr = 16, g.temporary = !0, i.push(g), o[g.entryName] = g, d.add(g);
        }
    }
    function I() {
      ...
      for (var A = a.offset, D = 0; D < i.length; D++) {
        var g = A, m = new t(u, s);
        m.header = s.slice(g, g += n.Constants.CENHDR), m.entryName = s.slice(g, g += m.header.fileNameLength), m.header.extraLength && (m.extra = s.slice(g, g += m.header.extraLength)), m.header.commentLength && (m.comment = s.slice(g, g + m.header.commentLength)), A += m.header.centralHeaderSize, i[D] = m, o[m.entryName] = m;
      }
      d.clear(), h();
    }
    function b(A) {
      var D = s.length - n.Constants.ENDHDR, g = Math.max(0, D - 65535), m = g, v = s.length, y = -1, w = 0;
      for ((typeof u.trailingSpace == "boolean" ? u.trailingSpace : !1) && (g = 0), D; D >= m; D--)
        if (s[D] === 80) {
          if (s.readUInt32LE(D) === n.Constants.ENDSIG) {
            y = D, w = D, v = D + n.Constants.ENDHDR, m = D - n.Constants.END64HDR;
            continue;
          }
          ...
          if (s.readUInt32LE(D) === n.Constants.ZIP64SIG) {
            y = D, v = D + n.readBigUInt64LE(s, D + n.Constants.ZIP64SIZE) + n.Constants.ZIP64LEAD;
            break;
          }
        }
    ...
    function C() {
      i.length > 1 && !f && i.sort((A, D) => A.entryName.toLowerCase().localeCompare(D.entryName.toLowerCase()));
    }
    return {
      /**
       ...
       * @returns {void}
       */
      deleteFile: function(A, D = !0) {
        l || I();
        const g = o[A];
        this.getEntryChildren(g, D).map((v) => v.entryName).forEach(this.deleteEntry);
      },
      /**
       ...
       * @return Array
       */
      getEntryChildren: function(A, D = !0) {
      ...
      getChildCount: function(A) {
        if (A && A.isDirectory) {
          const D = this.getEntryChildren(A);
          return D.includes(A) ? D.length - 1 : D.length;
        }
        return 0;
      },
      /**
      ...
      compressToBuffer: function() {
        ...
        for (const E of this.entries) {
          ...
          m += U, A.push($), A.push(k);
          const x = E.packCentralHeader();
          D.push(x), a.size += x.length, g += U + x.length, v++;
        }
      ...
      toAsyncBuffer: function(A, D, g, m) {
        try {
          l || I(), C();
          const v = [], y = [];
          let w = 0, E = 0, k = 0;
          ...
          const $ = function(U) {
            if (U.length > 0) {
              const x = U.shift(), T = x.entryName + x.extra.toString();
              g && g(T), x.getCompressedDataAsync(function(O) {
                m && m(T), x.header.offset = E;
                const M = x.packLocalHeader(), K = M.length + O.length;
                E += K, v.push(M), v.push(O);
                const G = x.packCentralHeader();
                y.push(G), a.size += G.length, w += K + G.length, k++, $(U);
              });
            } else {
              w += a.mainHeaderSize, a.offset = E, a.totalEntries = k, E = 0;
              const x = Buffer.alloc(w);
              v.forEach(function(O) {
                O.copy(x, E), E += O.length;
              }), y.forEach(function(O) {
                O.copy(x, E), E += O.length;
              });
              const T = a.toBinary();
              c && c.copy(T, T.length - c.length), T.copy(x, E), s = x, l = !1, A(x);
            }
          };
  ...
  return Bt = function(a, l) {
    ...
    return {
      ...
      addLocalFileAsync: function(m, v) {
        ...
        const $ = this;
        f.fs.stat(y, function(U, x) {
          if (U) return v(U, !1);
          E = E ? A(E) : "";
          const T = e.win32.basename(e.win32.normalize(y));
          if (E += k || T, x.isFile())
            f.fs.readFile(y, function(O, M) {
              return O ? v(O, !1) : ($.addFile(E, M, w, x), setImmediate(v, void 0, !0));
            });
          else if (x.isDirectory())
            return E += f.sep, $.addFile(E, Buffer.alloc(0), w, x), setImmediate(v, void 0, !0);
        });
      },
      /**
      ...
      addLocalFolder: function(m, v, y) {
        if (y = D(y), v = v ? A(v) : "", m = e.normalize(m), f.fs.existsSync(m)) {
      ...
      addLocalFolderAsync: function(m, v, y, w) {
        ...
        f.fs.open(m, "r", function(k) {
          if (k && k.code === "ENOENT")
            ...
            var $ = f.findFiles(m), U = -1, x = function() {
              if (U += 1, U < $.length) {
                var T = $[U], O = g(m, T).split("\\").join("/");
                O = O.normalize("NFD").replace(/[\u0300-\u036f]/g, "").replace(/[^\x20-\x7E]/g, ""), w(O) ? f.fs.stat(T, function(M, K) {
                  M && v(void 0, M), K.isFile() ? f.fs.readFile(T, function(G, B) {
                    G ? v(void 0, G) : (E.addFile(y + O, B, "", K), x());
                  }) : (E.addFile(y + O + "/", Buffer.alloc(0), "", K), x());
                }) : process.nextTick(() => {
                  x();
                });
              } else
      ...
      addLocalFolderAsync2: function(m, v) {
        ...
        k instanceof RegExp ? k = /* @__PURE__ */ (function(T) {
          return function(O) {
            return T.test(O);
          };
        })(k) : typeof k != "function" && (k = function() {
          return !0;
        }), E = E ? A(E) : "", $ === "latin1" && ($ = (T) => T.normalize("NFD").replace(/[\u0300-\u036f]/g, "").replace(/[^\x20-\x7E]/g, "")), typeof $ != "function" && ($ = (T) => T);
        const U = (T) => e.join(E, $(g(w, T))), x = (T) => e.win32.basename(e.win32.normalize($(T)));
        f.fs.open(w, "r", function(T) {
          T && T.code === "ENOENT" ? v(void 0, t.Errors.FILE_NOT_FOUND(w)) : T ? v(void 0, T) : f.findFilesAsync(w, function(O, M) {
            if (O) return v(O);
      ...
      addFile: function(m, v, y, w) {
        ...
        let x = E.isDirectory ? 16384 : 32768;
        return $ ? x |= 4095 & w.mode : typeof w == "number" ? x |= 4095 & w : x |= E.isDirectory ? 493 : 420, U = (U | x << 16) >>> 0, E.attr = U, E.setData(v), k || p.setEntry(E), E;
      },
      /**
      ...
      extractEntryTo: function(m, v, y, w, E, k) {
        ...
        var U = h($.entryName), x = I(v, k && !$.isDirectory ? h(k) : y ? U : e.basename(U));
        if ($.isDirectory) {
          var T = p.getEntryChildren($);
      ...
      extractAllToAsync: function(m, v, y, w) {
        ...
        m = e.resolve(m);
        const E = (x) => I(m, e.normalize(h(x.entryName))), k = (x, T) => new Error(x + ': "' + T + '"'), $ = [], U = [];
        p.entries.forEach((x) => {
          x.isDirectory ? $.push(x) : U.push(x);
        });
        for (const x of $) {
          const T = E(x), O = y ? x.header.fileAttr : void 0;
          try {
            f.makeDir(T), O && f.fs.chmodSync(T, O), f.fs.utimesSync(T, x.header.time, x.header.time);
          } catch {
            w(k("Unable to create folder", T));
          }
        }
        U.reverse().reduce(function(x, T) {
          return function(O) {
            if (O)
              x(O);
            else {
              const M = e.normalize(h(T.entryName)), K = I(m, M);
              T.getDataAsync(function(G, B) {
                if (B)
                  x(B);
                else if (!G)
                  x(t.Errors.CANT_EXTRACT_FILE());
                else {
                  const V = y ? T.header.fileAttr : void 0;
                  f.writeFileToAsync(K, G, v, V, function(J) {
                    J || x(k("Unable to write file", K)), f.fs.utimes(K, T.header.time, T.header.time, function(ve) {
                      ve ? x(k("Unable to set times", K)) : x();
                    });
                  });
                }
              });
      ...
      writeZipPromise: function(m, v) {
        ...
        return new Promise((E, k) => {
          !m && u.filename && (m = u.filename), m || k("ADM-ZIP: ZIP File Name Missing"), this.toBufferPromise().then(($) => {
            const U = (x) => x ? E(x) : k("ADM-ZIP: Wasn't able to write zip file");
...
async function Ws(t, e, n) {
  ...
  const d = on(t);
  for (const T of t) {
    const O = String(T.metadata.conversationId ?? T.id);
    T.metadata.classification = d.get(O);
  }
  let u = await It(e) + 1;
  ...
  let D = 0;
  for (const T of t) {
    D++, (v = n == null ? void 0 : n.onProgress) == null || v.call(n, Math.round(D / A * 100));
    const O = String(T.metadata.conversationId ?? T.id), M = d.get(O) ?? rn(T);
    T.metadata.classification = M;
    const K = f.get(O), G = qs(M);
    let B, V;
    K ? (B = K, V = !0, (y = n == null ? void 0 : n.log) == null || y.call(n, "info", `Updating existing source ${B} for conversation ${O}`)) : (B = St(u), u++, V = !1);
    const J = Vs(B, T), ve = S.join(e, "Sources", G, J), Ie = `Sources/${G}/${J}`, Le = Qi(B, T, M), Pe = Zs(B, T), rt = S.join(e, "ExecutiveSessions", G, Pe), Fe = `ExecutiveSessions/${G}/${Pe}`, it = qi(B, T, M, Ie), P = {
      krcId: B,
      conversationId: O,
      title: T.title,
    ...
    try {
      ...
      f.set(O, B), z === "created" ? (i++, r.push(B), p.push(hn(B, T.title, Le, M.uncertain ? "Review Needed" : M.primaryCategory))) : z === "updated" && (P.status = "updated"), await Yn(rt, it, V), o++, (E = n == null ? void 0 : n.log) == null || E.call(n, "info", `Executive session: ${Fe}`), M.uncertain ? (l++, P.status = P.status === "updated" ? "updated" : "uncertain", C.uncertain.push(P)) : (a++, C.classified.push(P)), (k = n == null ? void 0 : n.log) == null || k.call(n, "info", `${z === "created" ? "Created" : "Updated"} [${M.primaryCategory}] ${B}: ${Ie}`), b || (await ro(I, T, n), b = !0);
    } catch (z) {
  ...
  try {
    ...
    s.push(`Registry/review update: ${O}`), (x = n == null ? void 0 : n.log) == null || x.call(n, "error", O);
  }
...
async function ao(t, e, n) {
  ...
  if (m) {
    ...
    Array.isArray(w) && (D = w.length, g.push(...w.map((E) => E.fileName).filter(Boolean)));
  }
  for (const w of t) {
    const E = String(w.metadata.conversationId ?? w.id), k = u.get(E) ?? rn(w), $ = p.get(E), U = qs(k);
    let x, T;
    $ ? (x = $, T = "update") : (x = St(f), f++, T = "create");
    const O = Vs(x, w), M = S.join(h, U, O), K = `Sources/${U}/${O}`, G = Zs(x, w), B = `ExecutiveSessions/${U}/${G}`;
    if (T === "create")
      try {
        await R.access(M), T = "skip";
      } catch {
      }
    T === "create" ? c++ : T === "update" ? a++ : T === "skip" && l++, k.uncertain && d++, o.push({
      conversationId: E,
      title: w.title,
      krcId: x,
      action: T,
...
async function we(t) {
  ...
  for (const b of o) {
    const C = S.basename(b), A = rs(t, b), D = C.match(ns);
    if (!D) {
      q(e, {
        severity: "info",
        category: "info",
  ...
  const f = pi(e), h = {
    ready: !(f.errors.length > 0) && await pe(t),
    statusLevel: "healthy",
    statusHeadline: "",
    statusSubline: "",
    repositoryPath: t,
...
function yo(t) {
  ...
  return {
    ...
    createTime: ge(e, "Create Time"),
    updateTime: ge(e, "Update Time")
  };
}
async function nr(t) {
  const n = (await Ct(t)).filter((r) => r.category === "sources" && sr(r.name)), s = [];
  for (const r of n) {
    let i = "";
    try {
...
function yn(t) {
  ...
  return {
    krcId: e[1],
    title: e[2].trim(),
    conversationId: ge(s, "ChatGPT Conversation ID"),
    createTime: ge(s, "Create Time"),
    updateTime: ge(s, "Update Time"),
    description: ge(s, "Description"),
    fileReferences: wo(ge(s, "File References")),
    messages: i
  };
}
function wn(t, e) {
...
const xe = ".kae-index", Co = "evidence-index.json", ar = 1;
function Et(t) {
  return S.join(t, xe, Co);
}
async function cr(t) {
  const e = Et(t);
  try {
...
async function Qe(t) {
  const e = await Ct(t), n = [], s = await rr(t), r = s ? await ir(t, s) : null;
  for (const c of e) {
...
function Fo(t, e, n) {
  ...
  if (t.kind === "message" && t.message) {
    ...
    (f || p > 0) && (r += f ? 30 : p * 8, s.add("message"), s.add("keyword"), To(t.message.role) && (s.add("prompt"), f && (r += 10)), _o(t.message.role) && (s.add("response"), f && (r += 10)));
  }
  if (t.kind === "executive_session") {
    const u = ct(t);
...
async function Bo(t, e, n = 50) {
  const s = e.toLowerCase(), r = await Ct(t), i = [];
  for (const o of r) {
...
async function ra(t, e, n, s) {
  ...
  n.value += 1;
  const l = St(n.value), d = (/* @__PURE__ */ new Date()).toISOString().slice(0, 10), u = sa(i, l, d), f = ea(c, i, l, u), p = S.join(t, "Sources", a.categoryFolder), h = na(l, a.title), I = S.join(p, h), b = `Sources/${a.categoryFolder}/${h}`;
  ...
  const A = Er(l, a.title), D = S.join(C, A), g = `ExecutiveSessions/${a.categoryFolder}/${A}`, m = Sr(l, b, { ...a, title: a.title }, `Generated by KAE Repository Repair after reassigning duplicate ${i} → ${l}.`);
  return await R.writeFile(D, m, "utf8"), await pn(t, [
...
async function ca(t, e = {}) {
  const n = e.log ?? (() => {
  }), s = e.sessionId ?? `repair-${crypto.randomUUID()}`, r = t.repositoryPath, i = await we(r);
  ...
  const c = t.actions.filter((h) => h.autoRepairSafe && !h.manualReviewRequired), a = t.actions.length - c.length, d = { value: await It(r) }, u = [], f = [];
  for (const h of c)
    try {
      let I;
      switch (h.type) {
...
function ga(t, e) {
  ...
  return t.message.text.toLowerCase().includes(n) || Te(e).some((s) => t.message.text.toLowerCase().includes(s));
}
function ya(t, e) {
  if (!e || !t.attachment)
    return !1;
  const n = e.toLowerCase(), s = t.attachment.filename.toLowerCase();
  return s.includes(n) || Te(e).some((r) => s.includes(r));
}
...
function Cr(t, e, n) {
  var E, k, $, U, x, T, O, M, K, G, B, V, J, ve, Ie, Le, Pe, rt, Fe, it;
  const s = fa(t, e);
  if (!s)
    return null;
  const { krcId: r, sourcePath: i } = da(s), o = r ? la(t, r) : [s], c = (r ? ma(o, r) : void 0) ?? t.records.find((P) => P.kind === "source" && P.repository.repositoryPath === i) ?? (s.kind === "source" ? s : void 0), a = r ? pa(o, r) : void 0, l = a ?? c, d = ha(t, r), u = ((E = a == null ? void 0 : a.conversation) == null ? void 0 : E.title) ?? ((k = l == null ? void 0 : l.conversation) == null ? void 0 : k.title) ?? (($ = s.conversation) == null ? void 0 : $.title) ?? ((U = c == null ? void 0 : c.conversation) == null ? void 0 : U.title) ?? "Untitled", f = o.filter((P) => P.kind === "message").sort((P, z) => {
    ...
    oe(u, i, {
      ...
      subtitle: [
        (x = l.conversation) == null ? void 0 : x.conversationId,
        (T = l.conversation) != null && T.created ? `Created ${l.conversation.created}` : void 0,
        (O = l.conversation) != null && O.updated ? `Updated ${l.conversation.updated}` : void 0
      ].filter(Boolean).join(" · "),
      highlighted: s.kind === "conversation"
    })
  ] : [], "No conversation metadata indexed."), D = ie("messages", "Messages", f.map((P) => {
    var z, Y;
    ...
    return oe(`${Y === "image" ? "Screenshot" : Y === "video" ? "Video" : "File"}: ${z}`, i, {
      recordId: P.id,
      kind: "attachment",
      subtitle: (Tn = P.attachment) != null && Tn.resolved ? P.attachment.assetPath : "Unresolved reference",
  ...
  if (y.push({
    ...
    subtitle: (ve = d.session) == null ? void 0 : ve.linkedKrcId,
    timestamp: (Ie = d.conversation) == null ? void 0 : Ie.created,
  ...
  return {
    ...
    async reason(c, a) {
      ...
      try {
        ...
        if (!f.ok)
          return { ...be(c, t, l), usedOfflineFallback: !0 };
        ...
        } : { ...be(c, t, l), usedOfflineFallback: !0 };
      } catch {
        return { ...be(c, t, l), usedOfflineFallback: !0 };
      }
    }
...
const Da = et("openai", "OpenAI", "gpt-4o-mini", () => "https://api.openai.com/v1/chat/completions", (t, e) => ({
  model: e,
  messages: [
...
}, !0), xa = et("claude", "Claude", "claude-3-5-haiku-latest", () => "https://api.anthropic.com/v1/messages", (t, e) => ({
  model: e,
  max_tokens: 1024,
...
}, !0), Ta = et("gemini", "Gemini", "gemini-1.5-flash", (t) => `https://generativelanguage.googleapis.com/v1beta/models/${t.model ?? "gemini-1.5-flash"}:generateContent?key=${t.apiKey ?? ""}`, (t) => ({
...
function Re(t) {
  return t.reasonStream ? t : {
    ...
    async reasonStream(e, n, s) {
      ...
      return ht(r.providerId, "direct_answer", r.directAnswer, s), ht(r.providerId, "summary", r.reasonedSummary, s), s({ kind: "done", text: "", providerId: r.providerId }), r;
    }
  };
}
function Oa(t) {
  return {
    ...
    async reasonStream(e, n, s) {
      const r = be(e, "mock");
      return ht("mock", "direct_answer", r.directAnswer, s), ht("mock", "summary", r.reasonedSummary, s), s({ kind: "done", text: "", providerId: "mock" }), Fa("mock", r.directAnswer, r.reasonedSummary, !0);
    }
  };
}
class Ua {
  constructor(e) {
    ...
    for (const n of e ?? Ma())
      this.providers.set(n.capabilities.id, Re(n));
  }
  register(e) {
...
function qa(t) {
  ...
  for (const m of d) {
    const v = He(((D = m.attachment) == null ? void 0 : D.filename) ?? m.excerpt), y = u.get(v) ?? [];
    y.push(m), u.set(v, y);
  }
...
function kt(t) {
  return S.join(t, xe, Wa);
}
async function kr(t) {
  try {
    const e = await R.readFile(kt(t), "utf8"), n = JSON.parse(e);
...
async function qe(t) {
  const e = await kr(t);
  return e && e.repositoryPath === t ? e : De(t);
}
function tc(t) {
  const e = {};
...
function Xt(t, e, n = 50) {
  const s = Te(e), r = e.toLowerCase(), i = [];
...
function fc(t) {
  return t.kind === "executive_session" ? 92 : Rt(Sn(t)) ? 78 : 65;
}
function us(t, e) {
...
function mc(t) {
  const e = t.filter((r) => {
    const i = Sn(r);
    return r.kind === "executive_session" || Rt(i);
  }).sort((r, i) => gt(i) - gt(r)).slice(0, 4), n = e[0], s = n ? fc(n) : 50;
  return {
...
function pc(t, e) {
  const n = e.issues.filter((a) => a.severity === "error" || a.severity === "warning"), s = t.filter((a) => In(Sn(a))).sort((a, l) => gt(l) - gt(a)).slice(0, 4), r = [
    ...n.slice(0, 2).map(Dr),
    ...s.map(En)
...
function vc(t) {
  ...
  return {
    ...
    confidence: e,
    evidenceLinks: t.issues.length > 0 ? t.issues.slice(0, 4).map(Dr) : [
      {
...
const Ic = "executive-briefing-cache.json", Tr = 1;
function _r(t) {
  return S.join(t, xe, Ic);
}
async function Sc(t) {
  const e = Et(t), n = kt(t), [s, r] = await Promise.all([
    cr(t),
    kr(t)
  ]);
  let i = 0, o = 0;
...
async function Cc(t, e) {
  try {
    if ((await R.stat(Et(t))).mtimeMs > e.evidenceIndexMtimeMs)
      return !0;
  } catch {
    return !0;
  }
  try {
    if ((await R.stat(kt(t))).mtimeMs > e.relationshipIndexMtimeMs)
      return !0;
  } catch {
    return !0;
  }
...
function Fr(t) {
  return S.join(bt(t), "manifest.json");
}
function Qt(t, e) {
  return S.join(bt(t), "sessions", `${e}.json`);
}
function Or(t, e) {
  return S.join(bt(t), "archive", `${e}.json`);
}
async function Ur(t) {
  const e = bt(t);
...
function Ac(t, e, n) {
  ...
  if (e === "decision" && (t.kind === "executive_session" && (r += 45, s.push("executive session")), t.kind === "message" && ((d = t.message) != null && d.role.toLowerCase().includes("assistant")) && (r += 20, s.push("assistant response")), Rt(i) && (r += 25, s.push("decision language")), r += ds(t), ds(t) > 0 && s.push("recent evidence")), e === "summarize" && ((t.kind === "conversation" || t.kind === "source") && (r += 30, s.push("conversation source")), t.kind === "executive_session" && (r += 25, s.push("session summary"))), e === "show_evidence") {
...
function Dc(t, e, n = 30) {
  var u;
  const s = Ar(e), r = cc(e), i = Te(r), o = lr(t, r, 80), c = new Map(t.records.map((f) => [f.id, f])), a = [];
  for (const f of o) {
...
function Lc(t, e) {
  const { intent: n, searchQuery: s, queryTerms: r, items: i } = Dc(t, e), o = xc(t, i, s).filter((u) => u !== null), c = i.filter((u) => u.kind === "executive_session");
...
function fe(t, e = 220) {
  var r, i;
  const n = t.trim().replace(/^#+\s*/gm, "").replace(/\*\*([^*]+)\*\*/g, "$1").replace(/\s+/g, " ");
  if (!n)
    return "";
  if (n.length <= e)
    return n;
...
function Oc(t, e) {
  if (e.level === "insufficient" || t.items.length === 0)
    return `I found limited grounded evidence for "${t.searchQuery}".`;
  const n = Kr(t);
  switch (t.intent) {
    case "decision": {
      const r = t.executiveSessions[0];
      if (r)
        return `Executive session "${r.title}" records a decision tied to your question — ${fe(r.excerpt)}`;
      const i = t.messages.find((o) => {
        var c;
        return (c = o.messageRole) == null ? void 0 : c.toLowerCase().includes("assistant");
      });
      if (i)
        return `The strongest decision signal is in "${i.title}" — ${fe(i.excerpt)}`;
      break;
    }
    case "summarize": {
      const r = t.topKrcIds.slice(0, 3).join(", ") || "indexed sources";
      return `Across ${t.items.length} retrieved record(s) (${r}), the through-line is — ${fe(t.items.slice(0, 3).map((i) => i.excerpt).join(" "), 280)}`;
    }
    case "show_evidence": {
      const r = t.attachments[0];
      return r ? `Attachment "${r.title}" surfaced as the primary evidence — ${fe(r.excerpt)}` : `Primary evidence is in "${n.title}" — ${fe(n.excerpt)}`;
    }
    case "blockers": {
      const r = t.items.filter((i) => /blocker|unresolved|remaining|issue|risk|todo|pending|missing/i.test(i.excerpt));
      return r.length === 0 ? `No explicit blocker language appeared for "${t.searchQuery}" — closest matches may be incomplete.` : `${r.length} blocker-related hit(s); lead item "${r[0].title}" — ${fe(r[0].excerpt)}`;
    }
  }
  const s = t.topKrcIds.length > 1 ? ` with ${t.topKrcIds.length} corroborating sources` : "";
  return `"${n.title}" is the lead finding${s} — ${fe(n.excerpt)}`;
}
function Uc(t, e) {
  ...
  switch (t.intent) {
    ...
    default:
      return `This connects "${t.searchQuery}" to indexed repository knowledge you can act on.`;
  }
}
function Mc(t, e) {
  var i;
  ...
  const n = Kr(t), s = t.executiveSessions[0], r = (i = t.items.find((o) => o.explorerPath)) == null ? void 0 : i.explorerPath;
  return t.intent === "blockers" ? n ? `Open "${n.title}" and decide whether to add this blocker to your active campaign tracker.` : "Ask me which campaign this blocker affects, then we can trace ownership." : t.intent === "decision" && s ? `Continue the "${s.title}" thread — ask what was decided, or open it in Explorer to verify.` : s ? `Pick up the executive session on "${s.title}" and ask what we should do next there.` : n != null && n.explorerPath ? `Open "${n.title}" in Explorer, then ask me to explain how it affects your current objective.` : r ? "Open the lead source in Explorer and ask me to connect it to your current campaign." : `Ask me to go deeper on "${t.searchQuery}" or name the campaign you want to steer toward.`;
}
function jc(t, e) {
  const n = t.searchQuery, s = Gt(Oc(t, e)), r = Gt(Uc(t, e)), i = Gt(Mc(t, e));
  return e.level === "insufficient" ? [
    `I looked through what we have on "${n}", but the evidence is thin.`,
    s,
    r,
...
function Gt(t) {
  return t.replace(/^#+\s*/gm, "").replace(/\*\*([^*]+)\*\*/g, "$1").replace(/\s+/g, " ").trim();
}
function Bc(t, e) {
  if (t.items.length === 0)
    return "No supporting records to cite — expand Search if you need raw hits.";
  const n = t.topKrcIds.slice(0, 4).join(", ");
  return `${e.level === "high" ? "High confidence" : e.level === "medium" ? "Medium confidence" : e.level === "low" ? "Lower confidence" : "Limited confidence"} — I am drawing on ${t.items.length} indexed record(s)${n ? ` across ${n}` : ""}. Say the word if you want me to open any of them with you.`;
}
function Kc(t) {
  return {
    recordId: t.recordId,
...
function zt(t, e) {
  ...
  for (const r of t)
    if (!s.has(r.recordId) && (s.add(r.recordId), n.push(Kc(r)), n.length >= e))
      break;
  return n;
}
function Gc(t) {
...
class Vc {
  compose(e) {
    const n = Gc(e), s = zt(e.items, 8), r = zt(e.attachments, 6), i = zt(e.relatedSources, 5);
    return {
      question: e.question,
      intent: e.intent,
      searchQuery: e.searchQuery,
      directAnswer: zc(e, n),
      reasonedSummary: Hc(e, n),
      evidenceUsed: s,
      confidence: n,
      timeline: e.timeline,
      relatedSources: i,
...
async function Gr(t, e, n, s) {
  const r = await _e(t), i = Lc(r, e), o = Wc(i), c = Fc(n), a = c != null && c.conversationId ? await At(t, c.conversationId) : await jr(t), l = await Lr(t), d = Pc({
    repositoryPath: t,
    question: e,
    evidence: i,
    conversation: c,
...
async function Jc(t, e, n, s) {
  return Gr(t, e, { ...n, streaming: !0 }, s);
}
const Xc = "conversations", Qc = "active.json";
function kn(t) {
  return S.join(t, ".kae-sessions", Xc);
}
function Rn(t, e) {
  return S.join(kn(t), `${e}.json`);
}
function bn(t) {
  return S.join(kn(t), Qc);
}
async function eu(t) {
  const e = kn(t);
...
async function nu(t) {
  const e = await zr(t);
  return e ? tu(t, e.conversationId) : null;
}
...
const ou = /campaign\s+([\d.]+[a-z]?)\s*(?:[—–-]\s*([^\n.?]+)|(?=\s|$))/gi, ls = /\b(completed?|finished|implemented|shipped|passed|pass\b|done with)\b/i, au = /\b(import(?:ed)?|repair(?:ed)?|index(?:ed)?|rebuilt|snapshot|rollback)\b/i;
function me(t, e, n) {
  return {
    id: W(),
    label: t.trim(),
...
function du(t, e) {
  ...
  if (r && (s.currentCampaign = r), t.role === "user" && t.question && !e.currentObjective && (s.currentObjective = t.question.trim()), t.role === "assistant" && t.answer) {
    ...
    (c === "decision" || Rt(o)) && (s.currentDecisions = Se(e.currentDecisions, [
      me(t.answer.directAnswer.slice(0, 160), t.question, t.turnId)
    ])), (c === "blockers" || In(o)) && (s.currentBlockers = Se(e.currentBlockers, [
      me(t.answer.directAnswer.slice(0, 160), t.question, t.turnId)
    ])), (ls.test(o) || ls.test(t.displayText)) && (s.currentAccomplishments = Se(e.currentAccomplishments, [
      me(t.answer.directAnswer.slice(0, 160), t.question, t.turnId)
    ]));
    const a = t.answer.evidenceUsed.map((d) => me(d.label, d.recordId, t.turnId));
    s.currentEvidence = Se(e.currentEvidence, a);
    const l = [
      ...t.answer.explorerLinks.map((d) => me(d.label, d.path, t.turnId)),
      ...t.answer.attachments.map((d) => me(d.label, d.explorerPath, t.turnId))
    ];
    s.currentFiles = Se(e.currentFiles, l), t.followUpContext && (s.followUpContext = t.followUpContext);
  }
  if (au.test(n)) {
    const o = ((i = t.question) == null ? void 0 : i.trim()) || n.slice(0, 120);
    s.currentRepositoryChanges = Se(e.currentRepositoryChanges, [
      me(o, void 0, t.turnId)
    ]);
  }
  return s;
}
...
function mu(t, e) {
  ...
  return s != null && s.followUpContext && (n.followUpContext = s.followUpContext), n.unfinishedWork = lu(n), n.recommendedNextAction = fu(n), n;
}
const pu = "KAE";
function hu(t) {
...
async function gu(t, e, n) {
  const s = hu(n.conversationId), r = Xe(n.title || "Vigsy_Conversation"), i = `${s}_${r}_SESSION.md`, o = `ExecutiveSessions/${pu}/${i}`, c = S.join(t, o), a = `.kae-sessions/conversations/${n.conversationId}.json`, l = n.turns.filter((f) => f.role === "user" && f.question).slice(-3).map((f) => `- **Q:** ${f.question}`), d = [...n.turns].reverse().find((f) => f.role === "assistant");
...
`) : "Active Vigsy conversation session.",
    "",
    ...Ee("Current Decisions", e.currentDecisions),
    ...Ee("Current Blockers", e.currentBlockers),
    ...Ee("Current Accomplishments", e.currentAccomplishments),
    ...Ee("Current Files", e.currentFiles),
    ...Ee("Current Evidence", e.currentEvidence),
    ...Ee("Repository Changes", e.currentRepositoryChanges),
    "## Action / Follow-up",
...
function wu(t, e = Pr) {
  ...
  const n = yu(t.updatedAt, /* @__PURE__ */ new Date()), s = [`Welcome back${e ? ` ${e}` : ""}.`];
...
async function ms(t, e) {
  const n = await At(t, e.conversationId);
  if (n)
    return (n.lifecycle === "paused" || n.lifecycle === "closed") && (n.lifecycle = "active", n.updatedAt = (/* @__PURE__ */ new Date()).toISOString(), await We(t, n), await yt(t, n)), n;
  const s = Br(e.conversationId, e.title);
  return await We(t, s), await yt(t, s), s;
}
async function vu(t) {
  const e = await jr(t);
  !e || e.lifecycle !== "active" || (e.lifecycle = "paused", e.pausedAt = (/* @__PURE__ */ new Date()).toISOString(), e.updatedAt = e.pausedAt, await We(t, e), await yt(t, e, !1));
}
async function Iu(t, e) {
...
async function Su(t, e) {
  const n = await At(t, e.conversationId) ?? Br(e.conversationId, e.title), s = mu(e, {
    ...n,
    lifecycle: e.turns.length > 0 ? "active" : n.lifecycle
  });
  s.executiveSessionPath = await gu(t, s, e), await We(t, s), await yt(t, s);
  const r = await Qe(t), i = await De(t), o = await tt(t);
  return {
    session: s,
...
async function Eu(t) {
  const e = await nt(t);
  let n = null;
  return e.activeSessionId && (n = await Cn(t, e.activeSessionId)), wu(n, e.founderName);
}
const Cu = /^(user|assistant|system|human|chatgpt|cursor)\s*:\s*(.*)$/i, ku = /^(user|assistant|system)$/i;
function Ru(t) {
  var r;
  const e = t.trim();
  if (!e)
...
async function Du(t, e) {
  const n = Ru(e.transcript);
  if (n.length === 0)
    throw new Error("Transcript is empty.");
  await Hs(t);
  const s = await It(t) + 1, r = St(s), i = bu(e.title, n, e.sourceKind), o = Xe(i), c = `${r}_${o}.md`, a = `${r}_${o}_SESSION.md`, l = `Sources/${dt}/${c}`, d = `ExecutiveSessions/${dt}/${a}`, u = Au(r, i, e, n), f = $u(r, i, l, e, n);
  await R.mkdir(S.join(t, "Sources", dt), { recursive: !0 }), await R.mkdir(S.join(t, "ExecutiveSessions", dt), {
    recursive: !0
  }), await R.writeFile(S.join(t, l), u, "utf8"), await R.writeFile(S.join(t, d), f, "utf8");
  const p = await Qe(t), h = await De(t), I = await tt(t);
  return {
    krcId: r,
...
async function xu(t) {
  const e = await Qe(t), n = await De(t), s = await tt(t);
  return {
...
async function Ht() {
  return (await import("./index-BJOGQpfX.js")).chatGptConnector;
}
class Lu extends st {
  constructor() {
    super(...arguments);
...
const Pu = new Lu(), Fu = /(?:youtube\.com\/watch\?v=|youtu\.be\/|youtube\.com\/embed\/)([A-Za-z0-9_-]{11})/, Ou = /[?&]list=([A-Za-z0-9_-]+)/, Uu = /youtube\.com\/channel\/([A-Za-z0-9_-]+)/;
function gs(t) {
  const e = t.match(Fu);
  if (e)
...
function Bu(t) {
  return [...t.matchAll(/<text[^>]*>([\s\S]*?)<\/text>/g)].map((n) => ju(n[1].replace(/\n/g, " ").trim())).filter(Boolean).join(`
`);
}
async function Ku(t) {
  try {
    const n = (await Ye(`https://www.youtube.com/api/timedtext?type=list&v=${t}`)).match(/lang_code="([^"]+)"/), s = (n == null ? void 0 : n[1]) ?? "en", r = await Ye(`https://www.youtube.com/api/timedtext?v=${t}&lang=${s}`), i = Bu(r);
    if (i.trim())
      return i;
  } catch {
  }
...
async function Gu(t, e) {
  var l;
  const n = `https://www.youtube.com/watch?v=${t}`, s = await Mu(`https://www.youtube.com/oembed?url=${encodeURIComponent(n)}&format=json`);
  let r = "", i, o;
  try {
  ...
  const c = await Ku(t), a = c.length > 0;
  return {
    videoId: t,
    title: s.title,
    description: r,
...
async function Is(t, e) {
  ...
  const i = [];
  try {
    const p = await Ce(`/repos/${n}/${s}/readme`, e.token), h = Buffer.from(p.content, "base64").toString("utf8");
    i.push({
      kind: "readme",
      id: `${t}#readme`,
      title: `${s} README`,
  ...
  if (e.includeIssues)
    try {
      const p = await Ce(`/repos/${n}/${s}/issues?state=all&per_page=10`, e.token);
      for (const h of p)
  ...
  if (e.includePullRequests)
    try {
      const p = await Ce(`/repos/${n}/${s}/pulls?state=all&per_page=10`, e.token);
      for (const h of p)
        i.push({
          kind: "pull_request",
          id: `${t}#pr-${h.number}`,
  ...
  if (e.includeCommits)
    try {
      const p = await Ce(`/repos/${n}/${s}/commits?per_page=10`, e.token);
      for (const h of p)
        i.push({
          kind: "commit",
  ...
  if (e.includeReleases)
    try {
      const p = await Ce(`/repos/${n}/${s}/releases?per_page=10`, e.token);
      for (const h of p)
        i.push({
          kind: "release",
          id: `${t}#release-${h.id}`,
...
class Wu extends st {
  ...
  async syncInternal(n, s) {
    ...
    const i = qu(s), o = Es(s.config.settings);
    if (o.length > 0)
...
const Yu = new Wu(), Zr = [".md", ".markdown", ".txt"];
async function Vr(t) {
  const e = [];
  let n;
  try {
...
const Qu = ne("claude", "Claude Connector", "Acquire knowledge from Claude conversation exports.", ["export", "session"]), ed = ne("cursor", "Cursor Connector", "Acquire knowledge from Cursor workspace sessions.", ["workspace", "session"]), td = ne("google-drive", "Google Drive Connector", "Acquire documents from Google Drive folders.", ["folder", "file"]), nd = ne("onedrive", "OneDrive Connector", "Acquire documents from OneDrive folders.", ["folder", "file"]), sd = ne("notion", "Notion Connector", "Acquire pages and databases from Notion workspaces.", ["page", "database"]), rd = ne("obsidian", "Obsidian Connector", "Acquire vault notes from Obsidian.", ["vault"]), id = ne("rss", "RSS Connector", "Acquire feed entries from RSS/Atom sources.", ["feed"]), od = ne("websites", "Websites Connector", "Acquire content from configured website URLs.", ["url", "sitemap"]), ad = ne("podcasts", "Podcasts Connector", "Acquire show notes and transcripts from podcast feeds.", ["feed", "episode"]), cd = [
  Qu,
  ed,
  td,
  nd,
...
class ld {
  constructor() {
    ...
    _(this, "scheduler");
    _(this, "worker");
    this.scheduler = new _u((e, n) => this.monitorConnector(e, n)), this.worker = new Nu(this.scheduler);
    for (const e of dd)
  ...
  async syncNow(e, n, s, r) {
    ...
    try {
      if (u = await i.sync(n, d), u.documents.length > 0) {
        const b = await Tu(this.repositoryPath, u.documents);
        if (u.itemsSkipped += u.documents.length - b.documents.length, u.documents = b.documents, b.documents.length > 0) {
          const C = await Ws(b.documents, this.repositoryPath, {
            ...
            log: (D, g) => {
              var m;
              return (m = d.log) == null ? void 0 : m.call(d, D, g);
            },
            onProgress: (D) => {
              var g;
              return (g = d.onProgress) == null ? void 0 : g.call(d, "emit", D);
            }
          });
          ...
          const A = await xu(this.repositoryPath);
...
const wt = new fd(), md = 500, pd = /* @__PURE__ */ new Set(["system"]);
function hd(t) {
...
function Wr(t) {
  var c;
  const e = t.conversation_id ?? t.id ?? Cd(), n = (((c = t.title) == null ? void 0 : c.trim()) || "Untitled Conversation").slice(0, 200), r = wd(t).map((a) => Id(a.message)).filter((a) => a !== null), i = r.filter((a) => a.isPastedTranscript).map((a) => a.text), o = [...new Set(r.flatMap((a) => a.fileReferences))];
  return {
    conversationId: e,
    title: n,
...
function Id(t) {
  ...
  return {
    ...
    isPastedTranscript: e === "user" && n.length >= md,
    fileReferences: s
  };
}
function Sd(t) {
...
function kd(t) {
  ...
  for (const n of t) {
    ...
    if (e.push(`### ${s}`), n.createTime && e.push(`*${Rd(n.createTime)}*`), e.push(""), e.push(n.text), e.push(""), n.fileReferences.length > 0) {
...
function Yr(t, e, n = {}) {
  const { sharedAssetList: s } = n, r = kd(t.messages), i = t.pastedTranscripts.length > 0 ? `
...
const bd = "conversations.json", Ad = [
...
function Jr(t, e = {}) {
  ...
  (p = r == null ? void 0 : r.onZipOpened) == null || p.call(r, o.length), lt(s);
  const c = o.find((b) => !b.isDirectory && b.entryName.replace(/\\/g, "/").endsWith(bd));
  if (!c)
  ...
  (h = r == null ? void 0 : r.onConversationsJsonLocated) == null || h.call(r, a, l), lt(s);
  const d = c.getData().toString("utf8"), u = [];
  let f = 0;
  for (const b of o) {
    if (lt(s), b.isDirectory)
      continue;
    const C = b.entryName.replace(/\\/g, "/"), A = S.basename(C);
    if (Ad.some((g) => g.test(A) || g.test(C)) || C.endsWith(".json") && !C.includes("/"))
      continue;
    f++;
    const D = {
      zipPath: C,
...
class $d {
  ...
  async extract(e) {
    const n = Jr(e.source.path, { loadAssetData: !1 }), s = gd(n.conversationsJson);
    return {
      connectorId: this.id,
      source: e.source,
      rawDocuments: s,
...
async function Od(t, e) {
  ...
  if (await R.mkdir(An(), { recursive: !0 }), $n()) {
    ...
    await R.writeFile(vt(t), s), await R.writeFile(As(), "secure", "utf8");
    return;
  }
  await R.writeFile(vt(t), Buffer.from(n, "utf8").toString("base64"), "utf8"), await R.writeFile(As(), "dev-fallback", "utf8");
}
async function ti(t) {
  try {
    const e = await R.readFile(vt(t));
...
process.env.VITE_DEV_SERVER_URL && process.on("message", (t) => {
  t === "electron-vite&type=hot-reload" && Gd();
});
let ce = null, ee = null, Be = null, ri = null, ye = null, Wt = null, Yt = null;
const ue = gi(), Z = new wi(), ft = [];
function F() {
...
function j(t, e, n, s) {
  ...
  return ft.unshift(r), ft.length > 500 && ft.pop(), L == null || L.webContents.send("kae:log-added", r), r;
}
async function Ke() {
  try {
...
function Hd() {
  bs.forEach((t) => wt.register(t)), bi.register(Ys), j("info", "system", `Registered ${bs.length} connector plugin(s)`);
}
function xs() {
  L = new nn({
    width: 1280,
...
async function Vd(t, e) {
  ...
  ii(c), await pt(`Confirm import started: ${s} (${t})`), H(c, "validate-zip", "running"), j("info", "import", `Pre-import validation gate: ${s}`);
  let a;
  try {
    ce != null && ce.valid && ce.filePath === t && ee && Be === t ? (a = ce, await pt(
  ...
  H(c, "create-snapshot", "complete", S.basename(u)), j("info", "repository", `Snapshot saved: ${u}`), H(c, "analyze-export", "running"), await pt(
  ...
  const A = Date.now() - n, D = wt.get(e), g = {
    ...
    durationMs: A,
    connectorId: e,
    connectorName: (D == null ? void 0 : D.name) ?? e,
...
function qd() {
  N.handle(
  ...
  ), N.handle("kae:get-repository-config", () => ue.repository), N.handle("kae:set-repository-config", async (t, e) => (ue.repository = e, await te(), j("info", "repository", `Repository path set to ${e.path}`), ue.repository)), N.handle("kae:get-settings", () => Ae()), N.handle("kae:set-settings", async (t, e) => (ue.settings = Dn(e), await Bd(Ae()), await he(), j("info", "settings", "Application settings updated"), Ae())), N.handle("kae:get-jobs", () => Z.getAll()), N.handle("kae:get-logs", () => ft), N.handle("kae:get-default-repository-path", () => Us), N.handle("kae:get-repository-health", async () => we(F())), N.handle("kae:get-git-readiness", async () => (await we(F())).gitReadiness), N.handle("kae:get-repository-stats", async () => hr(F())), N.handle("kae:browse-repository", async () => Ct(F())), N.handle(
    ...
    async (t, e, n) => {
      const s = mt(e), r = await R.readFile(s), i = wn(r, n ?? S.basename(e)), o = e.replace(/\\/g, "/");
      return {
    ...
    async (t, e, n) => {
      const s = await Ds(n);
      return await he(), Yc(F(), e, s);
    }
  ), N.handle(
    "kae:answer-knowledge-question-stream",
    async (t, e, n) => {
      const s = await Ds({ ...n, streaming: !0 });
      return await he(), Jc(F(), e, s, (r) => {
  ...
  }), N.handle("kae:get-provider-health", async () => (await he(), ze().testActiveProviderHealth())), N.handle("kae:get-provider-key-status", async () => ({
    secureStorage: ei(),
    providers: await Md()
  })), N.handle("kae:set-provider-api-key", async (t, e, n) => (await Od(e, n), await he(), ni(e))), N.handle("kae:capture-live-session", async (t, e) => {
    const n = await Du(F(), e);
    ...
    return e.turns.length > 0 && (await Su(F(), e), L == null || L.webContents.send("kae:executive-memory-updated"), L == null || L.webContents.send("kae:executive-briefing-updated"), L == null || L.webContents.send("kae:vigsy-refreshed")), n;
  }), N.handle("kae:create-vigsy-conversation", async () => {
    await vu(F());
    const t = await ru(F());
    return await ms(F(), t), t;
  }), N.handle("kae:delete-vigsy-conversation", async (t, e) => {
    await Iu(F(), e), await iu(F(), e);
  }), N.handle("kae:get-executive-continuity", async () => Eu(F())), N.handle("kae:open-repository-path", async () => {
    await $t.openPath(F());
  }), N.handle("kae:open-repository-file", async (t, e) => {
    await $t.openPath(mt(e));
  }), N.handle("kae:reveal-repository-file", async (t, e) => {
    $t.showItemInFolder(mt(e));
    ...
    return Zd(e);
  }), N.handle("kae:cancel-validation", () => ye ? (ye.abort(), j("warn", "import", "Validation cancelled by user — repository unchanged"), oi({
    status: "cancelled",
    stage: "cancelled",
    ...
    if (!e || !e.toLowerCase().endsWith(".zip"))
      throw new Error("Please select a valid .zip file. Repository unchanged.");
    return Vd(e, "chatgpt-export-zip");
...
$e.whenReady().then(async () => {
  ue.settings = Dn(await jd()), await he(), Ts.handle("kae-asset", async (t) => {
    const e = Kd(t.url), n = mt(e), s = await R.readFile(n), r = wn(s, S.basename(e));
    return new Response(s, { headers: { "Content-Type": r } });
  }), Hd(), await te(), qd(), j("info", "system", `${Ps} started — ${Fs}`), xs(), $e.on("activate", () => {
    nn.getAllWindows().length === 0 && xs();
  });
});
