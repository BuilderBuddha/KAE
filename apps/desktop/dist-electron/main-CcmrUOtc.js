...
import { safeStorage, app, protocol, BrowserWindow, ipcMain, shell, clipboard, dialog } from "electron";
...
function stageLog(context, stage, message) {
  var _a;
  (_a = context.log) == null ? void 0 : _a.call(context, "info", `[${stage}] ${message}`);
}
...
async function runConnectorPipeline(connector, source, context) {
  const reportProgress = (stage, progress) => {
    var _a;
    (_a = context.onProgress) == null ? void 0 : _a.call(context, stage, progress);
  };
  const checkAborted = () => {
    var _a;
  ...
  const discovered = await connector.discover(source);
  checkAborted();
  reportProgress("extract", 20);
  stageLog(context, "extract", "Extracting raw content");
  const extracted = await connector.extract(discovered);
  checkAborted();
  reportProgress("normalize", 40);
  stageLog(context, "normalize", "Normalizing documents");
  const normalized = await connector.normalize(extracted);
...
const pipeline = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  emitImportPackage,
  generateProvenance,
...
async function writeAxiomSources(documents, repositoryPath, context) {
  ...
  for (const doc of documents) {
    ...
    const sessionPath2 = path.join(repositoryPath, "ExecutiveSessions", categoryFolder, sessionFilename);
    ...
    try {
      ...
      await safeWriteFile(sessionPath2, sessionMarkdown, overwrite);
      sessionsCreated++;
...
async function loadConnectorState(repositoryPath) {
...
async function saveConnectorState(repositoryPath, state) {
...
async function writeSessionManifest(repositoryPath, manifest) {
  ...
  const manifestPath2 = path.join(sessionsDir, `${manifest.sessionId}.json`);
  await fs.writeFile(manifestPath2, JSON.stringify(manifest, null, 2), "utf8");
...
async function executeReassignKrcId(repositoryPath, action, nextKrcCounter, log) {
  ...
  const sessionPath2 = path.join(sessionDir, sessionFileName);
  ...
  await fs.writeFile(sessionPath2, sessionContent, "utf8");
...
async function executeGenerateSession(repositoryPath, action, log) {
  ...
  const sessionPath2 = path.join(sessionDir, sessionFileName);
  const sessionRelative = `ExecutiveSessions/${parsed.categoryFolder}/${sessionFileName}`;
  if (await fileExists(sessionPath2)) {
    return {
      actionId: action.id,
      type: action.type,
      success: false,
...
function isSteeringAnswer(text) {
  return /what changed\s*[—-]/i.test(text) && /what i recommend next\s*[—-]/i.test(text);
}
function offlineStyledResponse(request, providerId, model) {
  ...
  const directAnswer = /^here'?s what i found/i.test(direct) ? direct.replace(/^here'?s what i found:?\s*/i, `${prefix} `) : `${prefix} ${direct}`;
  return {
    providerId,
    model: model ?? "offline",
    directAnswer,
...
function buildCuratedPrompt(request) {
  const { context, groundedAnswer } = request;
  const evidenceLines = groundedAnswer.evidenceUsed.slice(0, 8).map((item) => `- ${item.label}: ${item.excerpt}`);
  return [
...
async function parseJsonAnswer(text) {
...
function createHttpProvider(id, displayName, defaultModel, endpoint, buildBody, extractText, requiresApiKey) {
  return {
    capabilities: {
      id,
      displayName,
    ...
    async reason(request, credentials) {
      const model = (credentials == null ? void 0 : credentials.model) ?? defaultModel;
      if (!(credentials == null ? void 0 : credentials.apiKey) && requiresApiKey) {
        return offlineStyledResponse(request, id, model);
      }
      const prompt = buildCuratedPrompt(request);
      try {
        const headers2 = { "Content-Type": "application/json" };
        if (id === "claude" && (credentials == null ? void 0 : credentials.apiKey)) {
          headers2["x-api-key"] = credentials.apiKey;
          headers2["anthropic-version"] = "2023-06-01";
        } else if (credentials == null ? void 0 : credentials.apiKey) {
          headers2.Authorization = `Bearer ${credentials.apiKey}`;
        }
...
const openAiProvider = createHttpProvider("openai", "OpenAI", "gpt-4o-mini", () => "https://api.openai.com/v1/chat/completions", (prompt, model) => ({
  model,
  messages: [
  ...
  var _a, _b, _c;
  const data = payload;
  return ((_c = (_b = (_a = data.choices) == null ? void 0 : _a[0]) == null ? void 0 : _b.message) == null ? void 0 : _c.content) ?? null;
}, true);
const claudeProvider = createHttpProvider("claude", "Claude", "claude-3-5-haiku-latest", () => "https://api.anthropic.com/v1/messages", (prompt, model) => ({
  model,
  max_tokens: 1024,
  ...
  var _a, _b;
  const data = payload;
  return ((_b = (_a = data.content) == null ? void 0 : _a[0]) == null ? void 0 : _b.text) ?? null;
}, true);
const geminiProvider = createHttpProvider("gemini", "Gemini", "gemini-1.5-flash", (credentials) => `https://generativelanguage.googleapis.com/v1beta/models/${credentials.model ?? "gemini-1.5-flash"}:generateContent?key=${credentials.apiKey ?? ""}`, (prompt) => ({
  contents: [{ parts: [{ text: prompt }] }]
}), (payload) => {
  var _a, _b, _c, _d, _e;
  const data = payload;
  return ((_e = (_d = (_c = (_b = (_a = data.candidates) == null ? void 0 : _a[0]) == null ? void 0 : _b.content) == null ? void 0 : _c.parts) == null ? void 0 : _d[0]) == null ? void 0 : _e.text) ?? null;
}, true);
const openRouterProvider = createHttpProvider("openrouter", "OpenRouter", "openai/gpt-4o-mini", () => "https://openrouter.ai/api/v1/chat/completions", (prompt, model) => ({
  model,
  messages: [{ role: "user", content: prompt }]
}), (payload) => {
  var _a, _b, _c;
...
const ollamaProvider = createHttpProvider("ollama", "Local (Ollama)", "llama3.2", (credentials) => `${credentials.baseUrl ?? "http://127.0.0.1:11434"}/api/chat`, (prompt, model) => ({
  model,
  stream: false,
  messages: [{ role: "user", content: prompt }]
}), (payload) => {
  var _a;
  const data = payload;
  return ((_a = data.message) == null ? void 0 : _a.content) ?? null;
}, false);
function healthForOfflineProvider(providerId, supportsStreaming, secureStorage) {
  return {
    providerId,
    status: "offline",
...
function healthMissingKey(providerId, supportsStreaming, secureStorage) {
  return {
    providerId,
    status: "missing_key",
...
async function probeOpenAiCompatible(providerId, url, apiKey, supportsStreaming, secureStorage) {
  try {
...
async function testProviderHealth(providerId, credentials, secureStorage, capabilities) {
  ...
  if (capabilities.requiresApiKey && !(credentials == null ? void 0 : credentials.apiKey)) {
  ...
  if (providerId === "openai" || providerId === "openrouter") {
    ...
    return probeOpenAiCompatible(providerId, url, (credentials == null ? void 0 : credentials.apiKey) ?? "", capabilities.supportsStreaming, secureStorage);
  }
  if (providerId === "ollama") {
    const base = (credentials == null ? void 0 : credentials.baseUrl) ?? "http://127.0.0.1:11434";
    try {
  ...
  if (!(credentials == null ? void 0 : credentials.apiKey)) {
    return healthMissingKey(providerId, capabilities.supportsStreaming, secureStorage);
  }
  return {
    providerId,
...
function streamTextAsTokens(providerId, field, text, onChunk) {
...
async function finalizeStreamResponse(providerId, directAnswer, reasonedSummary, usedOfflineFallback) {
  return {
    providerId,
    directAnswer,
    reasonedSummary,
...
function withStreamingFallback(provider) {
  if (provider.reasonStream)
    return provider;
  return {
    ...provider,
...
function createStreamingMockProvider(base) {
  return {
    ...base,
...
class AIProviderManager {
  constructor(seedProviders) {
    ...
    __publicField(this, "activeId", "mock");
    __publicField(this, "credentials", {});
    __publicField(this, "secureStorageMode", "dev_fallback");
  ...
  async reasonStream(request, onChunk) {
  ...
  async testActiveProviderHealth() {
    const provider = this.getActive();
    return testProviderHealth(provider.capabilities.id, this.credentials, this.secureStorageMode, provider.capabilities);
  }
  async testProviderHealth(providerId) {
    const provider = this.providers.get(providerId);
    if (!provider)
      throw new Error(`Unknown AI provider: ${providerId}`);
    return testProviderHealth(providerId, this.credentials, this.secureStorageMode, provider.capabilities);
  }
}
...
function createDefaultAIProviderManager() {
  return new AIProviderManager();
}
let sharedManager$1 = null;
function getSharedAIProviderManager() {
  if (!sharedManager$1) {
    sharedManager$1 = createDefaultAIProviderManager();
  }
  return sharedManager$1;
}
function verifyGroundedAnswer(skeleton, response) {
  return {
...
function recordTimestamp(record) {
  var _a, _b, _c;
  return parseTimestamp$1(((_a = record.message) == null ? void 0 : _a.timestamp) ?? ((_b = record.conversation) == null ? void 0 : _b.updated) ?? ((_c = record.conversation) == null ? void 0 : _c.created));
}
...
async function saveExecutiveMemoryManifest(repositoryPath, manifest) {
  await ensureDirs(repositoryPath);
  await fs.writeFile(manifestPath(repositoryPath), JSON.stringify(manifest, null, 2), "utf8");
}
async function loadExecutiveSessionByConversation(repositoryPath, conversationId) {
...
async function loadExecutiveSessionById(repositoryPath, sessionId) {
  for (const loader of [
    ...
    try {
      ...
      const parsed = JSON.parse(raw);
      if (parsed.sessionId && parsed.conversationId)
        return parsed;
    } catch {
    }
  }
...
async function loadActiveExecutiveSession(repositoryPath) {
  const manifest = await loadExecutiveMemoryManifest(repositoryPath);
  if (!manifest.activeSessionId)
    return null;
  return loadExecutiveSessionById(repositoryPath, manifest.activeSessionId);
}
async function saveExecutiveSession(repositoryPath, session, options) {
  await ensureDirs(repositoryPath);
  const target = (options == null ? void 0 : options.archive) ? archivePath(repositoryPath, session.sessionId) : sessionPath(repositoryPath, session.sessionId);
  await fs.writeFile(target, JSON.stringify(session, null, 2), "utf8");
  if (options == null ? void 0 : options.archive) {
    try {
...
function createEmptyExecutiveSession(conversationId, title = "Vigsy conversation") {
  const now = (/* @__PURE__ */ new Date()).toISOString();
  return {
...
async function upsertManifestSession(repositoryPath, session, setActive = true) {
  const manifest = await loadExecutiveMemoryManifest(repositoryPath);
  manifest.repositoryPath = repositoryPath;
  manifest.lastSyncedAt = (/* @__PURE__ */ new Date()).toISOString();
  if (setActive)
    manifest.activeSessionId = session.sessionId;
...
function buildReasoningContext(input) {
  ...
  return {
    ...
    campaign: memory == null ? void 0 : memory.currentCampaign,
    objective: memory == null ? void 0 : memory.currentObjective,
    blockers: [
      ...(memory == null ? void 0 : memory.currentBlockers.map((item) => item.label)) ?? [],
      ...(memory == null ? void 0 : memory.unfinishedWork) ?? []
    ],
    accomplishments: (memory == null ? void 0 : memory.currentAccomplishments.map((item) => item.label)) ?? [],
    repositorySummary: input.executiveBriefing ? `${input.executiveBriefing.evidenceRecordCount} evidence records; ${input.executiveBriefing.relationshipCount} relationships` : void 0
  };
}
function conversationContextFromOptions(options) {
  return options == null ? void 0 : options.conversationContext;
}
function gist(text, max = 220) {
  var _a, _b;
  const normalized = text.trim().replace(/^#+\s*/gm, "").replace(/\*\*([^*]+)\*\*/g, "$1").replace(/\bKRC-\d+\b/gi, "").replace(/\s+/g, " ");
  if (!normalized)
    return "";
  if (normalized.length <= max)
    return normalized;
  const sentence = (_b = (_a = normalized.match(/^[^.!?]+[.!?]/)) == null ? void 0 : _a[0]) == null ? void 0 : _b.trim();
  if (sentence && sentence.length <= max)
    return sentence;
  return `${normalized.slice(0, max - 1).trim()}…`;
}
function sanitizeGist(text) {
  return text.replace(/^#+\s*/gm, "").replace(/\*\*([^*]+)\*\*/g, "$1").replace(/\bKRC-\d+\b/gi, "").replace(/\s+/g, " ").trim();
}
function leadItem(context) {
  return context.items[0];
}
function whereWeAre(context, confidence) {
  const topic = context.searchQuery;
  if (confidence.level === "insufficient") {
    return `We're early on "${topic}" — I need a bit more context before I can brief with confidence.`;
  }
  if (context.executiveSessions.length > 0) {
    return `We're working "${topic}" in line with your active executive sessions and recent decisions.`;
  }
  return `We're looking at "${topic}" — I'll keep this grounded in progress, decisions, and what matters next.`;
}
function lastSession(context) {
  const session = context.executiveSessions[0];
  if (!session)
    return null;
  const title = session.conversationTitle ?? session.title;
  const detail = gist(session.excerpt, 200);
  return detail ? `We were last focused on ${title}. ${detail}` : `We were last focused on ${title}.`;
}
function whatChanged(context, confidence) {
  ...
  const detail = gist(lead.excerpt, 240);
  switch (context.intent) {
    case "decision": {
      const session = context.executiveSessions[0];
      if (session) {
        return `A decision thread is active — ${gist(session.excerpt, 220)}`;
      }
      break;
    }
    case "summarize": {
      const throughLine = gist(context.items.slice(0, 3).map((item) => item.excerpt).join(" "), 260);
      return throughLine || "The through-line across recent work is becoming clearer.";
    }
    case "blockers": {
      ...
      if (blockerHits.length > 0) {
        return `${blockerHits.length} open thread(s) mention blockers or unresolved work — ${gist(blockerHits[0].excerpt, 180)}`;
      }
      break;
    }
  }
  const corroboration = context.topKrcIds.length > 1 ? ` ${context.topKrcIds.length} threads corroborate this.` : "";
  return detail ? `${detail}${corroboration}` : "Recent work on this topic has shifted since the last session.";
}
function whatNeedsAttention(context) {
  const tasks = [];
  const blockerHits = context.items.filter((item) => /blocker|unresolved|remaining|issue|risk|todo|pending|missing|attention|repair/i.test(item.excerpt));
  for (const hit of blockerHits.slice(0, 4)) {
    const line = gist(hit.excerpt, 140);
    if (line && !tasks.includes(line))
      tasks.push(line);
  }
  if (context.intent === "blockers" && tasks.length === 0) {
    tasks.push("Confirm whether any blocker still blocks the next executive move.");
  }
  if (tasks.length === 0) {
    return "• Nothing urgent is flagged — we can keep the investigation moving.";
  }
  return tasks.map((task) => `• ${task}`).join("\n");
}
function recommendNext(context, confidence) {
  if (confidence.level === "insufficient") {
  ...
  if (context.intent === "blockers") {
    return lead ? "Decide whether this blocker belongs on your active tracker, then assign an owner." : "Clarify which campaign this blocker affects so we can set the next move.";
  }
  if (context.intent === "decision" && session) {
    return `Close the loop on ${session.conversationTitle ?? session.title} — confirm what was decided and what follows.`;
  }
  if (session) {
    return `Pick up ${session.conversationTitle ?? session.title} and decide the next executive action.`;
  }
  if (lead) {
    return `Review the strongest thread on "${context.searchQuery}", then tell me which outcome you want to steer toward.`;
  }
  return `Tell me the outcome you want on "${context.searchQuery}" and I will keep the investigation pointed there.`;
}
function buildSteeringDirectAnswer(context, confidence) {
  const sections = [
    `Here's where we are — ${sanitizeGist(whereWeAre(context, confidence))}`
  ];
  const last = lastSession(context);
  if (last)
    sections.push(`Last session — ${sanitizeGist(last)}`);
  sections.push(`What's changed — ${sanitizeGist(whatChanged(context, confidence))}`);
  sections.push(`What needs attention —
${whatNeedsAttention(context)}`);
  sections.push(`Recommended next step — ${sanitizeGist(recommendNext(context, confidence))}`);
  return sections.join("\n\n");
}
function buildSteeringSupportingSummary(context, confidence) {
  if (context.items.length === 0) {
    return "Supporting awareness — no corroborating threads yet. Ask me to widen the search if you need more backup.";
  }
  const confidenceNote = confidence.level === "high" ? "Confidence is high" : confidence.level === "medium" ? "Confidence is moderate" : confidence.level === "low" ? "Confidence is lower" : "Confidence is limited";
  const threadCount = context.items.length;
  const sessionNote = context.executiveSessions.length > 0 ? ` ${context.executiveSessions.length} executive session(s) back this up.` : "";
  return `Supporting awareness — ${confidenceNote} across ${threadCount} corroborating thread(s).${sessionNote} Use the lenses below if you want sources, timeline, or repository detail.`;
}
...
function buildReasonedSummary(context, confidence) {
...
async function buildOrchestratedAnswer(repositoryPath, question, options, onChunk) {
  const index = await ensureEvidenceIndex(repositoryPath);
  ...
  const executiveMemory = (conversation == null ? void 0 : conversation.conversationId) ? await loadExecutiveSessionByConversation(repositoryPath, conversation.conversationId) : await loadActiveExecutiveSession(repositoryPath);
  ...
  const providerId = (options == null ? void 0 : options.providerId) ?? "mock";
  ...
  manager.setCredentials({
    ...
    streaming: options == null ? void 0 : options.streaming
  });
  const request = { context: reasoningContext, groundedAnswer: skeleton };
  const aiResponse = onChunk ? await manager.reasonStream(request, onChunk) : await manager.reason(request);
  const verified = verifyGroundedAnswer(skeleton, aiResponse);
  return enrichAnswerWithRelationships(repositoryPath, verified);
}
async function answerKnowledgeQuestion(repositoryPath, question, options) {
  return buildOrchestratedAnswer(repositoryPath, question, options);
}
async function answerKnowledgeQuestionStream(repositoryPath, question, options, onChunk) {
  return buildOrchestratedAnswer(repositoryPath, question, { ...options, streaming: true }, onChunk);
}
const CONVERSATIONS_DIR = "conversations";
const ACTIVE_CONVERSATION_FILE = "active.json";
function conversationsRoot(repositoryPath) {
  return path.join(repositoryPath, ".kae-sessions", CONVERSATIONS_DIR);
}
function conversationFilePath(repositoryPath, conversationId) {
  return path.join(conversationsRoot(repositoryPath), `${conversationId}.json`);
}
function activeStatePath(repositoryPath) {
  return path.join(conversationsRoot(repositoryPath), ACTIVE_CONVERSATION_FILE);
}
async function ensureConversationsDir(repositoryPath) {
  const dir = conversationsRoot(repositoryPath);
  await fs.mkdir(dir, { recursive: true });
  return dir;
}
async function loadActiveConversationState(repositoryPath) {
  try {
...
async function loadVigsyConversation(repositoryPath, conversationId) {
  try {
    const raw = await fs.readFile(conversationFilePath(repositoryPath, conversationId), "utf8");
    const parsed = JSON.parse(raw);
    if (!parsed.conversationId || !Array.isArray(parsed.turns))
      return null;
    return parsed;
  } catch {
    return null;
...
async function loadActiveVigsyConversation(repositoryPath) {
...
async function saveVigsyConversation(repositoryPath, record) {
...
function createEmptyVigsyConversation(title) {
  const now = (/* @__PURE__ */ new Date()).toISOString();
  return {
...
async function createNewVigsyConversation(repositoryPath) {
  const record = createEmptyVigsyConversation();
  await saveVigsyConversation(repositoryPath, record);
  return record;
}
async function deleteVigsyConversation(repositoryPath, conversationId) {
  try {
    await fs.unlink(conversationFilePath(repositoryPath, conversationId));
  } catch {
  }
  const active = await loadActiveConversationState(repositoryPath);
  if ((active == null ? void 0 : active.conversationId) === conversationId) {
    try {
...
const CAMPAIGN_PATTERN = /campaign\s+([\d.]+[a-z]?)\s*(?:[—–-]\s*([^\n.?]+)|(?=\s|$))/gi;
const ACCOMPLISHMENT_PATTERN = /\b(completed?|finished|implemented|shipped|passed|pass\b|done with)\b/i;
const REPO_CHANGE_PATTERN = /\b(import(?:ed)?|repair(?:ed)?|index(?:ed)?|rebuilt|snapshot|rollback)\b/i;
function memoryItem(label, detail, sourceTurnId) {
  return {
    id: randomUUID(),
    label: label.trim(),
    detail: detail == null ? void 0 : detail.trim(),
    sourceTurnId,
...
function extractCampaign(text) {
  ...
  const id = (_a = last[1]) == null ? void 0 : _a.trim();
  const name = (_b = last[2]) == null ? void 0 : _b.trim();
  if (!id)
    return void 0;
...
function turnText(turn) {
  var _a, _b;
  return [turn.question, turn.displayText, turn.supportingText, (_a = turn.answer) == null ? void 0 : _a.directAnswer, (_b = turn.answer) == null ? void 0 : _b.reasonedSummary].filter(Boolean).join("\n");
}
function extractFromTurn(turn, session) {
  var _a;
  const text = turnText(turn);
  const updates = {};
  const campaign = extractCampaign(text);
  if (campaign)
    updates.currentCampaign = campaign;
  ...
  if (turn.role === "assistant" && turn.answer) {
    ...
    const evidenceItems = turn.answer.evidenceUsed.map((item) => memoryItem(item.label, item.recordId, turn.turnId));
    updates.currentEvidence = mergeItems(session.currentEvidence, evidenceItems);
    const fileItems = [
      ...turn.answer.explorerLinks.map((link2) => memoryItem(link2.label, link2.path, turn.turnId)),
    ...
    if (turn.followUpContext)
      updates.followUpContext = turn.followUpContext;
  }
  if (REPO_CHANGE_PATTERN.test(text)) {
    const label = ((_a = turn.question) == null ? void 0 : _a.trim()) || text.slice(0, 120);
    updates.currentRepositoryChanges = mergeItems(session.currentRepositoryChanges, [
      memoryItem(label, void 0, turn.turnId)
    ]);
  }
  return updates;
}
function deriveUnfinishedWork(session) {
  const items = [];
  ...
  if (session.currentBlockers.length === 0) {
    for (const line of session.unfinishedWork) {
      if (/unfinished|remaining|blocker|not yet|todo/i.test(line))
        items.push(line);
    }
  }
  return [...new Set(items)].slice(0, 6);
}
function deriveRecommendedNextAction(session) {
  var _a;
  if (session.currentBlockers.length > 0) {
...
function extractExecutiveMemory(record, session) {
  ...
  if (lastAssistant == null ? void 0 : lastAssistant.followUpContext) {
...
function vigsyKrcId(conversationId) {
  return `VIGSY-${conversationId.slice(0, 8).toUpperCase()}`;
}
function listSection(title, items) {
  if (items.length === 0)
    return [`## ${title}`, "- None recorded yet.", ""];
  return [`## ${title}`, ...items.map((item) => `- ${item.label}${item.detail ? ` (${item.detail})` : ""}`), ""];
}
async function writeVigsyExecutiveSessionMarkdown(repositoryPath, session, record) {
  const krcId = vigsyKrcId(record.conversationId);
  const slug2 = slugifyTitle(record.title || "Vigsy_Conversation");
  const fileName = `${krcId}_${slug2}_SESSION.md`;
  ...
  const summaryLines = record.turns.filter((turn) => turn.role === "user" && turn.question).slice(-3).map((turn) => `- **Q:** ${turn.question}`);
  const lastAnswer = [...record.turns].reverse().find((turn) => turn.role === "assistant");
  if (lastAnswer == null ? void 0 : lastAnswer.displayText) {
...
function daysBetween(isoDate, now) {
  ...
  return Math.max(0, Math.round((start.getTime() - end.getTime()) / 864e5));
}
function timeReference(days) {
  if (days === 0)
...
function buildExecutiveContinuity(session, founderName = DEFAULT_FOUNDER_NAME) {
  ...
  const days = daysBetween(session.updatedAt, /* @__PURE__ */ new Date());
  const parts = [`Welcome back${founderName ? ` ${founderName}` : ""}.`];
  ...
  if (unfinishedCount > 0) {
    parts.push(unfinishedCount === 1 ? "One implementation remains unfinished." : `${unfinishedCount} implementations remain unfinished.`);
  }
...
async function ensureExecutiveSessionForConversation(repositoryPath, record) {
...
async function archiveExecutiveSessionForConversation(repositoryPath, conversationId) {
  ...
  const manifest = await loadExecutiveMemoryManifest(repositoryPath);
  manifest.sessions = manifest.sessions.map((item) => item.conversationId === conversationId ? { ...item, lifecycle: "archived", updatedAt: session.updatedAt } : item);
  if (manifest.activeSessionId === session.sessionId) {
    manifest.activeSessionId = void 0;
  }
  manifest.lastSyncedAt = session.updatedAt;
  await saveExecutiveMemoryManifest(repositoryPath, manifest);
}
async function syncExecutiveMemoryFromConversation(repositoryPath, record) {
  const base = await loadExecutiveSessionByConversation(repositoryPath, record.conversationId) ?? createEmptyExecutiveSession(record.conversationId, record.title);
  const session = extractExecutiveMemory(record, {
    ...base,
    lifecycle: record.turns.length > 0 ? "active" : base.lifecycle
  });
  session.executiveSessionPath = await writeVigsyExecutiveSessionMarkdown(repositoryPath, session, record);
  await saveExecutiveSession(repositoryPath, session);
  await upsertManifestSession(repositoryPath, session);
...
async function getExecutiveContinuity(repositoryPath) {
  const manifest = await loadExecutiveMemoryManifest(repositoryPath);
  let session = null;
  if (manifest.activeSessionId) {
...
function parseLiveTranscript(transcript) {
  var _a;
  ...
  if (/^###\s+(user|assistant)/im.test(trimmed)) {
    ...
    for (const block of trimmed.split(/^###\s+/im).slice(1)) {
      ...
      const roleRaw = (_a = lines[0]) == null ? void 0 : _a.trim().toLowerCase();
      if (!roleRaw || !BLOCK_ROLE.test(roleRaw))
        continue;
      const role = roleRaw === "user" ? "user" : roleRaw === "assistant" ? "assistant" : "system";
      messages2.push({ role, text: lines.slice(1).join("\n").trim() });
    }
  ...
  for (const line of trimmed.split("\n")) {
    ...
    if (match) {
      if (current == null ? void 0 : current.text.trim())
        messages.push(current);
      const roleToken = match[1].toLowerCase();
      const role = roleToken === "assistant" || roleToken === "chatgpt" || roleToken === "cursor" ? "assistant" : roleToken === "system" ? "system" : "user";
...
function deriveCaptureTitle(title, messages, sourceKind) {
  if (title == null ? void 0 : title.trim())
    return title.trim().slice(0, 120);
  const firstUser = messages.find((message) => message.role === "user");
  if (firstUser == null ? void 0 : firstUser.text.trim())
    return firstUser.text.trim().slice(0, 120);
  return `${sourceKind} session ${(/* @__PURE__ */ new Date()).toISOString().slice(0, 10)}`;
}
...
function buildLiveCaptureSourceMarkdown(krcId, title, input, messages) {
  var _a;
  const now = (/* @__PURE__ */ new Date()).toISOString();
...
${message.text.trim()}`;
  ...
  return [
    ...
    "",
    "## Notes",
    ((_a = input.notes) == null ? void 0 : _a.trim()) || "Captured via KAE live session capture.",
    "",
    "## Transcript",
    "",
...
function buildLiveCaptureExecutiveSessionMarkdown(krcId, title, sourceRelativePath, input, messages) {
  const summary = messages.slice(0, 4).map((message) => `- **${message.role}:** ${message.text.slice(0, 200)}`).join("\n");
  return [
    `# Executive Session Record — ${title}`,
    "",
    "## Source ID",
...
async function captureLiveSession(repositoryPath, input) {
  ...
  const slug2 = slugifyTitle(title);
  const sourceFileName = `${krcId}_${slug2}.md`;
  const sessionFileName = `${krcId}_${slug2}_SESSION.md`;
  ...
  const sessionMarkdown = buildLiveCaptureExecutiveSessionMarkdown(krcId, title, sourceRelativePath, input, messages);
  ...
  await fs.writeFile(path.join(repositoryPath, executiveSessionRelativePath), sessionMarkdown, "utf8");
  const evidenceIndex = await buildEvidenceIndex(repositoryPath);
  const relationshipIndex = await buildRelationshipIndex(repositoryPath);
  const briefing = await refreshExecutiveBriefing(repositoryPath);
  return {
...
async function refreshRepositoryAwareness(repositoryPath) {
  const evidenceIndex = await buildEvidenceIndex(repositoryPath);
  const relationshipIndex = await buildRelationshipIndex(repositoryPath);
  const briefing = await refreshExecutiveBriefing(repositoryPath);
  return {
...
async function deduplicateDocuments(repositoryPath, documents) {
...
class ConnectorScheduler {
  constructor(onSync) {
    ...
    __publicField(this, "schedules", /* @__PURE__ */ new Map());
    this.onSync = onSync;
  }
  load(schedules) {
    this.stop();
  ...
  enable(connectorId, intervalMinutes) {
    var _a;
    ...
    const schedule = {
      ...
      intervalMinutes,
      enabled: true,
      lastRunAt: (_a = this.schedules.get(connectorId)) == null ? void 0 : _a.lastRunAt,
      nextRunAt: new Date(Date.now() + intervalMinutes * 6e4).toISOString()
    };
  ...
  disable(connectorId) {
    ...
    const current = this.schedules.get(connectorId);
    if (current) {
      this.schedules.set(connectorId, { ...current, enabled: false, nextRunAt: void 0 });
    }
  }
  stop() {
...
class BackgroundConnectorWorker {
  ...
  schedule(connectorId, intervalMinutes) {
    this.scheduler.enable(connectorId, intervalMinutes);
  }
  unschedule(connectorId) {
...
function healthy(connectorId, message) {
  return {
    connectorId,
    status: "healthy",
    message,
...
class BaseConnector {
  ...
  async connect(_config, _context) {
    return healthy(this.id, `${this.name} connected`);
  }
  async disconnect() {
  }
  async checkHealth(_context) {
  ...
  async sync(source, context) {
    ...
    try {
      ...
      (_a = context.log) == null ? void 0 : _a.call(context, "error", message);
      return {
        success: false,
        connectorId: this.id,
        itemsImported: 0,
...
async function core() {
  const mod = await import("./index-ilT3GWP-.js");
  return mod.chatGptConnector;
}
class ChatGptFrameworkConnector extends BaseConnector {
  constructor() {
    super(...arguments);
  ...
  async syncInternal(source, context) {
    if (!source) {
      throw new Error("ChatGPT sync requires a ZIP export file.");
    }
    const { runConnectorPipeline: runConnectorPipeline2 } = await Promise.resolve().then(() => pipeline);
    const importPackage = await runConnectorPipeline2(this, source, context);
    return importPackage.documents;
  }
}
const chatGptFrameworkConnector = new ChatGptFrameworkConnector();
...
async function resolveVideoIds(target, maxVideos, context) {
  var _a;
  ...
  if (target.kind === "channel" && target.channelId) {
    ...
    (_a = context == null ? void 0 : context.log) == null ? void 0 : _a.call(context, "info", `Fetching channel feed: ${rssUrl}`);
  ...
  if (target.kind === "playlist" && target.playlistId) {
    const page = await fetchText(`https://www.youtube.com/playlist?list=${target.playlistId}`);
    const ids = [
      ...new Set([...page.matchAll(/"videoId":"([A-Za-z0-9_-]{11})"/g)].map((match) => match[1]))
    ];
    return ids.slice(0, maxVideos);
  }
...
function parseTimedTextXml(xml) {
  const lines = [...xml.matchAll(/<text[^>]*>([\s\S]*?)<\/text>/g)].map((match) => decodeEntities(match[1].replace(/\n/g, " ").trim()));
...
async function fetchYouTubeTranscript(videoId) {
  try {
    ...
    const lang = (langMatch == null ? void 0 : langMatch[1]) ?? "en";
    const transcriptXml = await fetchText(`https://www.youtube.com/api/timedtext?v=${videoId}&lang=${lang}`);
...
async function fetchYouTubeVideo(videoId, context) {
  var _a;
  ...
  try {
    ...
    publishDate = dateMatch == null ? void 0 : dateMatch[1];
    ...
    (_a = context == null ? void 0 : context.log) == null ? void 0 : _a.call(context, "warn", `Could not parse watch page for ${videoId}: ${String(err)}`);
  }
...
function buildYouTubeDocument(payload) {
...
function mockVideosFromSettings(settings) {
  const mockVideos = settings.mockVideos;
  return Array.isArray(mockVideos) ? mockVideos : [];
}
function changeKeysFromContext$1(context) {
...
class YouTubeConnector extends BaseConnector {
  constructor() {
    super(...arguments);
    __publicField(this, "id", "youtube");
    __publicField(this, "name", "YouTube Connector");
    __publicField(this, "description", "Acquire video metadata, transcripts, and captions from YouTube videos, playlists, and channels.");
    __publicField(this, "supportedExtensions", []);
    __publicField(this, "capabilities", {
      implementationStatus: "full",
      supportsScheduledSync: true,
      supportsOAuth: false,
  ...
  async extract(_discovered) {
    throw new Error("YouTube connector uses syncInternal.");
  }
  async normalize(_extracted) {
    throw new Error("YouTube connector uses syncInternal.");
  }
  async syncInternal(_source, context) {
    var _a, _b;
    const mockVideos = mockVideosFromSettings(context.config.settings);
    const changedKeys = changeKeysFromContext$1(context);
    if (mockVideos.length > 0) {
      return mockVideos.filter((video) => !changedKeys || changedKeys.has(`youtube:${video.videoId}`)).map((video) => buildYouTubeDocument(video));
    }
    const targetUrl = String(context.config.settings.targetUrl ?? context.config.settings.url ?? "").trim();
    if (!targetUrl) {
    ...
    const documents = [];
    const filteredVideoIds = changedKeys ? videoIds.filter((videoId) => changedKeys.has(`youtube:${videoId}`)) : videoIds;
    for (const videoId of filteredVideoIds) {
      (_a = context.log) == null ? void 0 : _a.call(context, "info", `Acquiring YouTube video ${videoId}`);
      const payload = await fetchYouTubeVideo(videoId, context);
      documents.push(buildYouTubeDocument(payload));
      (_b = context.onProgress) == null ? void 0 : _b.call(context, "extract", Math.round(documents.length / filteredVideoIds.length * 100));
    }
    return documents;
  }
  async detectChanges(context) {
    const checkedAt = (/* @__PURE__ */ new Date()).toISOString();
    const mockVideos = mockVideosFromSettings(context.config.settings);
    if (mockVideos.length > 0) {
      const keys2 = mockVideos.map((video) => `youtube:${video.videoId}`);
      const cursor = mockVideos.map((video) => `${video.videoId}:${video.title}:${video.publishDate ?? ""}`).join("|");
      return {
        connectorId: this.id,
        checkedAt,
        cursor,
...
async function ghFetch(path2, token) {
  ...
  if (token && token.trim()) {
    headers2.Authorization = `Bearer ${token.trim()}`;
  }
  const response = await fetch(`https://api.github.com${path2}`, { headers: headers2 });
  if (!response.ok) {
    throw new Error(`GitHub API ${response.status} for ${path2}`);
  }
  return response.json();
}
...
async function fetchGitHubRepository(repository, options) {
  ...
  try {
    ...
    (_a = options.log) == null ? void 0 : _a.call(options, "warn", `GitHub repo metadata unavailable, using fallback: ${String(err)}`);
    repoMeta = {
  ...
  const items = [];
  try {
    const readme = await ghFetch(`/repos/${owner}/${repo}/readme`, options.token);
    ...
    (_b = options.log) == null ? void 0 : _b.call(options, "warn", `README API unavailable, trying raw fallback: ${String(err)}`);
    try {
      ...
      (_c = options.log) == null ? void 0 : _c.call(options, "warn", `README unavailable: ${String(fallbackErr)}`);
    }
  }
  if (options.includeIssues) {
    try {
      const issues = await ghFetch(`/repos/${owner}/${repo}/issues?state=all&per_page=10`, options.token);
      for (const issue2 of issues) {
        if (issue2.html_url.includes("/pull/"))
          continue;
        items.push({
          kind: "issue",
          id: `${repository}#issue-${issue2.number}`,
          title: `Issue #${issue2.number}: ${issue2.title}`,
          body: issue2.body ?? "",
          url: issue2.html_url,
          updatedAt: issue2.updated_at,
          metadata: { number: issue2.number }
        });
      }
    } catch (err) {
      (_d = options.log) == null ? void 0 : _d.call(options, "warn", `Issues unavailable: ${String(err)}`);
    }
  }
  if (options.includePullRequests) {
    try {
      const pulls = await ghFetch(`/repos/${owner}/${repo}/pulls?state=all&per_page=10`, options.token);
      ...
      (_e = options.log) == null ? void 0 : _e.call(options, "warn", `Pull requests unavailable: ${String(err)}`);
    }
  }
  if (options.includeCommits) {
    try {
      const commits = await ghFetch(`/repos/${owner}/${repo}/commits?per_page=10`, options.token);
      ...
      (_f = options.log) == null ? void 0 : _f.call(options, "warn", `Commits unavailable: ${String(err)}`);
    }
  }
  if (options.includeReleases) {
    try {
      const releases = await ghFetch(`/repos/${owner}/${repo}/releases?per_page=10`, options.token);
      ...
      (_g = options.log) == null ? void 0 : _g.call(options, "warn", `Releases unavailable: ${String(err)}`);
    }
  }
  if (items.length === 0) {
...
function buildGitHubDocument(repository, item) {
...
function mockItemsFromSettings(settings) {
  const mockItems = settings.mockItems;
  return Array.isArray(mockItems) ? mockItems : [];
}
function changeKeysFromContext(context) {
  const keys = context.config.settings.__changeKeys;
  return Array.isArray(keys) ? new Set(keys.map(String)) : null;
}
class GitHubConnector extends BaseConnector {
  constructor() {
    super(...arguments);
    __publicField(this, "id", "github");
    __publicField(this, "name", "GitHub Connector");
    __publicField(this, "description", "Acquire repository README, issues, pull requests, commits, and releases from GitHub.");
    __publicField(this, "supportedExtensions", []);
    __publicField(this, "capabilities", {
      implementationStatus: "full",
      supportsScheduledSync: true,
  ...
  async syncInternal(_source, context) {
    ...
    if (mockItems.length > 0) {
      return mockItems.filter((item) => !changedKeys || changedKeys.has(`github:${item.id}`)).map((item) => buildGitHubDocument(repository, item));
    }
    ...
    return payload.items.filter((item) => !changedKeys || changedKeys.has(`github:${item.id}`)).map((item) => buildGitHubDocument(repository, item));
  }
  async detectChanges(context) {
    const checkedAt = (/* @__PURE__ */ new Date()).toISOString();
    const repository = String(context.config.settings.repository ?? "").trim();
    ...
    const mockItems = mockItemsFromSettings(context.config.settings);
    if (mockItems.length > 0) {
      const keys2 = mockItems.map((item) => `github:${item.id}`);
      const cursor2 = mockItems.map((item) => `${item.id}:${item.updatedAt ?? item.title}`).join("|");
      return {
        connectorId: this.id,
        checkedAt,
        cursor: cursor2,
    ...
    const cursor = payload.items.map((item) => `${item.id}:${item.updatedAt ?? item.title}`).join("|");
    return {
      connectorId: this.id,
      checkedAt,
      cursor,
...
function buildLocalFolderDocument(rootDir, filePath, content) {
...
class LocalFolderConnector extends BaseConnector {
  constructor() {
    super(...arguments);
    __publicField(this, "id", "local-folder");
    __publicField(this, "name", "Local Folder Connector");
    __publicField(this, "description", "Acquire knowledge from markdown and text files in a local directory.");
    __publicField(this, "supportedExtensions", SUPPORTED_EXTENSIONS);
    __publicField(this, "capabilities", {
      implementationStatus: "full",
      supportsScheduledSync: true,
      supportsOAuth: false,
  ...
  canHandle(file) {
    return this.supportedExtensions.some((ext) => {
      var _a;
      return ext === ((_a = file.extension) == null ? void 0 : _a.toLowerCase());
    });
  }
  async discover(source) {
    return {
  ...
  async syncInternal(source, context) {
    const folderPath = String((source == null ? void 0 : source.path) ?? context.config.settings.folderPath ?? "").trim();
    if (!folderPath) {
...
function stubConnector(id, name, description, acquisitionModes) {
  class StubConnector extends BaseConnector {
    constructor() {
      super(...arguments);
      __publicField(this, "id", id);
      __publicField(this, "name", name);
      __publicField(this, "description", description);
      __publicField(this, "supportedExtensions", []);
      __publicField(this, "capabilities", {
...
const claudeConnector = stubConnector("claude", "Claude Connector", "Acquire knowledge from Claude conversation exports.", ["export", "session"]);
const cursorConnector = stubConnector("cursor", "Cursor Connector", "Acquire knowledge from Cursor workspace sessions.", ["workspace", "session"]);
const googleDriveConnector = stubConnector("google-drive", "Google Drive Connector", "Acquire documents from Google Drive folders.", ["folder", "file"]);
const oneDriveConnector = stubConnector("onedrive", "OneDrive Connector", "Acquire documents from OneDrive folders.", ["folder", "file"]);
const notionConnector = stubConnector("notion", "Notion Connector", "Acquire pages and databases from Notion workspaces.", ["page", "database"]);
const obsidianConnector = stubConnector("obsidian", "Obsidian Connector", "Acquire vault notes from Obsidian.", ["vault"]);
const rssConnector = stubConnector("rss", "RSS Connector", "Acquire feed entries from RSS/Atom sources.", ["feed"]);
const websitesConnector = stubConnector("websites", "Websites Connector", "Acquire content from configured website URLs.", ["url", "sitemap"]);
const podcastsConnector = stubConnector("podcasts", "Podcasts Connector", "Acquire show notes and transcripts from podcast feeds.", ["feed", "episode"]);
...
const allConnectors = [...fullConnectors, ...stubConnectors];
function nowIso() {
  return (/* @__PURE__ */ new Date()).toISOString();
}
function defaultAuth(connector) {
...
class ConnectorManager {
  constructor() {
    ...
    __publicField(this, "onAwarenessRefreshed");
    __publicField(this, "scheduler");
    __publicField(this, "worker");
    this.scheduler = new ConnectorScheduler((connectorId, context) => this.monitorConnector(connectorId, context));
  ...
  async initialize(repositoryPath) {
    ...
    for (const config2 of Object.values(this.state.connectors)) {
      if (config2.connected && config2.scheduledSyncEnabled && config2.scheduleIntervalMinutes) {
        this.worker.schedule(config2.connectorId, config2.scheduleIntervalMinutes);
        this.state.monitoring[config2.connectorId] = {
          ...this.monitoringFor(config2.connectorId),
          active: true,
          nextCheckAt: this.nextCheckAt(config2.scheduleIntervalMinutes)
        };
      }
    }
  ...
  setAwarenessRefreshHandler(handler) {
  ...
  configFor(connectorId) {
    const state = this.ensureState();
    const connector = this.get(connectorId);
    return state.connectors[connectorId] ?? (connector == null ? void 0 : connector.getDefaultConfig()) ?? {
      connectorId,
      enabled: false,
      connected: false,
      scheduledSyncEnabled: false,
  ...
  async getStatuses(context) {
    ...
    const statuses = [];
    for (const connector of this.connectors.values()) {
      const config2 = this.configFor(connector.id);
      const totals = state.totals[connector.id] ?? { itemsImported: 0 };
      const health = config2.connected ? await connector.checkHealth(ctx) : {
        connectorId: connector.id,
        status: "disconnected",
  ...
  async connect(connectorId, config2, context) {
    const connector = this.get(connectorId);
    if (!connector)
  ...
  async disconnect(connectorId) {
    ...
    const config2 = this.configFor(connectorId);
    state.connectors[connectorId] = { ...config2, connected: false };
  ...
  async updateConfig(connectorId, config2) {
    const state = this.ensureState();
    const current = this.configFor(connectorId);
    const merged = { ...current, ...config2, connectorId };
    state.connectors[connectorId] = merged;
  ...
  async syncNow(connectorId, source, context, options) {
    var _a, _b;
    const connector = this.get(connectorId);
    if (!connector)
      throw new Error(`Unknown connector: ${connectorId}`);
    const state = this.ensureState();
    const config2 = (options == null ? void 0 : options.configOverride) ?? this.configFor(connectorId);
    if (!config2.connected) {
    ...
    (_a = ctx.log) == null ? void 0 : _a.call(ctx, "info", `${(options == null ? void 0 : options.automatic) ? "Auto-sync" : "Sync"} started: ${connector.name}`);
    let result;
    try {
      ...
      if (result.documents.length > 0) {
        ...
        if (deduped.documents.length > 0) {
          const exportResult = await writeAxiomSources(deduped.documents, this.repositoryPath, {
            importFileName: (source == null ? void 0 : source.name) ?? `${connectorId}-sync`,
            log: (level, message) => {
              var _a2;
              return (_a2 = ctx.log) == null ? void 0 : _a2.call(ctx, level, message);
            },
            onProgress: (progress) => {
              var _a2;
              return (_a2 = ctx.onProgress) == null ? void 0 : _a2.call(ctx, "emit", progress);
            }
          });
          result.itemsImported = exportResult.sourcesCreated;
          result.itemsUpdated = Math.max(deduped.updatedCount, exportResult.sourcesCreated === 0 && deduped.documents.length > 0 ? deduped.documents.length : 0);
          ...
          (_b = this.onAwarenessRefreshed) == null ? void 0 : _b.call(this);
        }
      }
  ...
  async monitorConnector(connectorId, context) {
    const connector = this.get(connectorId);
    if (!connector)
      throw new Error(`Unknown connector: ${connectorId}`);
    const state = this.ensureState();
    const config2 = this.configFor(connectorId);
    if (!config2.connected || !config2.scheduledSyncEnabled) {
      state.monitoring[connectorId] = {
        ...this.monitoringFor(connectorId),
        active: false,
        nextCheckAt: void 0
    ...
    try {
      ...
      const nextCheckAt = this.nextCheckAt(config2.scheduleIntervalMinutes);
      state.monitoring[connectorId] = {
        ...previous,
        active: true,
  ...
  async runPipeline(connectorId, source, context) {
    const connector = this.get(connectorId);
    if (!connector)
      throw new Error(`Unknown connector: ${connectorId}`);
    const started = Date.now();
  ...
  defaultMonitoringRecord(connectorId) {
    return {
      connectorId,
      active: false
    };
  }
  monitoringFor(connectorId) {
    const state = this.ensureState();
    return state.monitoring[connectorId] ?? this.defaultMonitoringRecord(connectorId);
  }
  nextCheckAt(intervalMinutes = 60) {
    return new Date(Date.now() + intervalMinutes * 6e4).toISOString();
  }
  appendEvent(state, connectorId, type, message, metadata) {
  ...
  async detectChanges(connector, context) {
...
function getSharedConnectorManager() {
  if (!sharedManager) {
...
function credentialPath(providerId) {
...
async function hasProviderApiKey(providerId) {
  ...
  return Boolean(key == null ? void 0 : key.trim());
}
async function loadActiveProviderApiKey(providerId) {
  if (providerId === "mock" || providerId === "deterministic") return void 0;
  return loadProviderApiKey(providerId);
}
async function listProviderKeyStatus() {
  const status = {};
...
async function loadPersistedSettings() {
  try {
    ...
    const parsed = JSON.parse(raw);
    const { aiApiKey: _removed, ...safe } = parsed;
    return safe;
  } catch {
    return {};
  }
...
async function ensureConnectorManager() {
  ...
  manager.setAwarenessRefreshHandler(() => {
    ...
    mainWindow == null ? void 0 : mainWindow.webContents.send("kae:vigsy-refreshed");
  });
}
function mergedSettings() {
  return mergeAppSettings(config.settings);
}
async function configureProviderManager() {
...
async function answerOptionsFromConfig(options) {
  const settings = mergedSettings();
  const providerId = (options == null ? void 0 : options.providerId) ?? settings.aiProvider;
  const apiKey = (options == null ? void 0 : options.apiKey) ?? await loadActiveProviderApiKey(providerId);
  return {
    providerId,
    apiKey,
    model: (options == null ? void 0 : options.model) ?? settings.aiModel,
    baseUrl: (options == null ? void 0 : options.baseUrl) ?? settings.aiBaseUrl,
    temperature: (options == null ? void 0 : options.temperature) ?? settings.aiTemperature,
    streaming: (options == null ? void 0 : options.streaming) ?? settings.aiStreaming,
    conversationContext: options == null ? void 0 : options.conversationContext
  };
}
...
async function runImport(filePath, formatId) {
  ...
  const manifestPath2 = await writeSessionManifest(repoPath(), {
    sessionId: jobId,
  ...
  addLog(
    ...
    { summary, snapshotPath, manifestPath: manifestPath2 }
  );
  mainWindow == null ? void 0 : mainWindow.webContents.send("kae:job-updated", jobQueue.getById(jobId));
...
function setupIpc() {
  ...
  ipcMain.handle("kae:set-repository-config", async (_event, repo) => {
  ...
  ipcMain.handle("kae:set-settings", async (_event, settings) => {
  ...
  ipcMain.handle("kae:test-ai-provider", async (_event, providerId) => {
    await configureProviderManager();
    const manager = getSharedAIProviderManager();
  ...
  ipcMain.handle("kae:set-provider-api-key", async (_event, providerId, apiKey) => {
    await saveProviderApiKey(providerId, apiKey);
    await configureProviderManager();
    return hasProviderApiKey(providerId);
  });
  ipcMain.handle("kae:capture-live-session", async (_event, input) => {
    const result = await captureLiveSession(repoPath(), input);
    void rebuildExecutiveBriefingCache();
    mainWindow == null ? void 0 : mainWindow.webContents.send("kae:executive-briefing-updated");
    mainWindow == null ? void 0 : mainWindow.webContents.send("kae:executive-memory-updated");
  ...
  ipcMain.handle("kae:save-vigsy-conversation", async (_event, record) => {
  ...
  ipcMain.handle("kae:delete-vigsy-conversation", async (_event, conversationId) => {
  ...
  ipcMain.handle("kae:connect-connector", async (_event, connectorId, config2) => {
    await ensureConnectorManager();
    return getSharedConnectorManager().connect(connectorId, config2, {
      repositoryPath: repoPath(),
      log: (level, message) => addLog(level, "connector", message)
    });
  });
  ipcMain.handle("kae:disconnect-connector", async (_event, connectorId) => {
    await ensureConnectorManager();
    await getSharedConnectorManager().disconnect(connectorId);
    return true;
  });
  ipcMain.handle("kae:update-connector-config", async (_event, connectorId, config2) => {
    await ensureConnectorManager();
    return getSharedConnectorManager().updateConfig(connectorId, config2);
  });
  ipcMain.handle("kae:sync-connector", async (_event, connectorId, sourcePath) => {
    await ensureConnectorManager();
  ...
  ipcMain.handle("kae:get-connector-sync-history", async (_event, connectorId) => {
    await ensureConnectorManager();
    return getSharedConnectorManager().getSyncHistory(connectorId);
  });
  ipcMain.handle("kae:get-connector-events", async (_event, connectorId) => {
    await ensureConnectorManager();
...
export {
  BaseImporter as B,
  ChatGptConnector as C,
  DocxImporter as D,
  HtmlImporter as H,
  ImporterRegistry as I,
  MarkdownImporter as M,
  PdfImporter as P,
  TxtImporter as T,
  ChatGptExportZipImporter as a,
  buildTranscriptMarkdown as b,
  chatGptConnector as c,
  isChatGptExport as d,
  extractChatGptZip as e,
  parseConversationsIncremental as f,
  parseConversationsJson as g,
  parseConversationsJsonArray as h,
  importerRegistry as i,
  parseConversation as p,
  runChatGptValidationPipeline as r,
  stubImporters as s,
  validateChatGptZipImport as v
};
