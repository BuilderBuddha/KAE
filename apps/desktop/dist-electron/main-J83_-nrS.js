...
async function core() {
  const mod = await import("./index-Cm1WJBH0.js");
...
class YouTubeConnector extends BaseConnector {
  ...
  async syncInternal(_source, context) {
    ...
    for (const videoId of videoIds) {
      ...
      (_b = context.onProgress) == null ? void 0 : _b.call(context, "extract", Math.round(documents.length / videoIds.length * 100));
    }
    return documents;
  }
}
...
class GitHubConnector extends BaseConnector {
  ...
  async syncInternal(_source, context) {
    ...
    const token = String(context.config.settings.apiKey ?? ((_a = context.auth) == null ? void 0 : _a.label) ?? "").trim();
    ...
    return payload.items.map((item) => buildGitHubDocument(repository, item));
  }
}
...
class ConnectorManager {
  constructor() {
    ...
    this.scheduler = new ConnectorScheduler((connectorId, context) => this.syncNow(connectorId, null, context));
  ...
  async initialize(repositoryPath) {
    ...
    this.scheduler.load(this.state.schedules);
  ...
  async updateConfig(connectorId, config2) {
    ...
    if (merged.scheduledSyncEnabled && merged.scheduleIntervalMinutes && merged.connected) {
      this.scheduler.enable(connectorId, merged.scheduleIntervalMinutes);
      state.schedules = this.scheduler.export();
    } else {
      this.scheduler.disable(connectorId);
      state.schedules = this.scheduler.export();
    }
  ...
  async syncNow(connectorId, source, context) {
    var _a;
    const connector = this.get(connectorId);
    if (!connector)
      throw new Error(`Unknown connector: ${connectorId}`);
    ...
    (_a = ctx.log) == null ? void 0 : _a.call(ctx, "info", `Sync started: ${connector.name}`);
    let result;
    try {
