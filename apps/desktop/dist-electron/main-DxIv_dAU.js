...
async function core() {
  const mod = await import("./index-najIEB78.js");
...
async function fetchGitHubRepository(repository, options) {
  ...
  const repoMeta = await ghFetch(`/repos/${owner}/${repo}`, options.token);
  const items = [];
  try {
    ...
    (_a = options.log) == null ? void 0 : _a.call(options, "warn", `README unavailable: ${String(err)}`);
  }
